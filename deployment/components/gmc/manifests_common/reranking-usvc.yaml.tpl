  ...
  template:
    ...
    spec:
          ...
          command:
            ...
            - |
                if [ -z "$RERANKING_SERVICE_ENDPOINT" ]; then
                  ...
                  HEALTH_URL=$(echo "$RERANKING_SERVICE_ENDPOINT" | sed 's|/v1$||')/health;
                  until curl -sf "$HEALTH_URL"; do
                    echo "waiting for reranking model server ${HEALTH_URL} to be ready...";
                    sleep 2;
                  done;
                fi;
      ...
      volumes:
        - name: tmp
          emptyDir: {}
{{- include "manifest.serviceMonitor" (list .filename "reranking-usvc" .) }}
