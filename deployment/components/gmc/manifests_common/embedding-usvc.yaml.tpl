  ...
  template:
    ...
    spec:
          ...
          command:
            ...
            - |
                if [ -z "$EMBEDDING_MODEL_SERVER_ENDPOINT" ]; then
                  ...
                  until curl -sf ${EMBEDDING_MODEL_SERVER_ENDPOINT}/health; do
                    echo "waiting for embedding model server ${EMBEDDING_MODEL_SERVER_ENDPOINT}/health to be ready...";
                    sleep 2;
                  done;
                fi;
      ...
      volumes:
        - name: tmp
          emptyDir: {}
{{- include "manifest.serviceMonitor" (list .filename "embedding-usvc" .) }}
