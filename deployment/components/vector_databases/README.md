...
Redis Cluster's configuration is managed through the environment's config file (`env/<name>/config.erag.yaml`):
...
Microsoft SQL Server's configuration is managed through the environment's config file (`env/<name>/config.erag.yaml`):

```yaml
vector_databases_enabled: true
vector_databases_vector_store: mssql
```
