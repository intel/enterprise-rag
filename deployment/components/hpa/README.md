...
- `metrics` section defines the name of the custom metric and the target value (threshold above which we scale up/down the deployment):
  ...
  - type: Object
    object:
      ...
      target:
        type: Value
        value: 2      # this value is a threshold, if measured value is above the target and scale policies are meet scaling replicas will proceed.
  ```
...
To watch all `HorizontalPodAutoscaler` objects across every namespace in real time:
