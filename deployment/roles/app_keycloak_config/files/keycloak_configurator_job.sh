...
MINIO_DOMAIN="${MINIO_DOMAIN:-minio.solutions.ai}"
...
# Set once create_mcp_client has run, so the credentials Secret is only warned about as
# missing MCP keys after there was actually an attempt to obtain them.
MCP_CONFIG_ATTEMPTED=false
...
store_credentials_to_secret() {
  ...
  if [[ "${MCP_ENABLED:-false}" == "true" ]]; then
    # MCP configuration is allowed to fail without failing the run. Fall back to the
    # already-stored secret in that case: the Keycloak-side secret has not changed, and
    # writing an empty value here would silently break every existing MCP caller.
    if [[ -z "$MCP_CLIENT_SECRET" ]] && kubectl get secret "$CREDENTIALS_SECRET_NAME" -n "$CREDENTIALS_SECRET_NAMESPACE" &>/dev/null; then
      MCP_CLIENT_SECRET=$(kubectl get secret "$CREDENTIALS_SECRET_NAME" -n "$CREDENTIALS_SECRET_NAMESPACE" \
        -o jsonpath="{.data.MCP_CLIENT_SECRET}" 2>/dev/null | base64 -d 2>/dev/null || echo "")
    fi

    if [[ -n "$MCP_CLIENT_SECRET" ]]; then
      extra_mcp_keys="  MCP_CLIENT_ID: $(echo -n "$MCP_CLIENT_ID" | base64 -w0)
  MCP_CLIENT_SECRET: $(echo -n "$MCP_CLIENT_SECRET" | base64 -w0)"
    elif [[ "$MCP_CONFIG_ATTEMPTED" == "true" ]]; then
      log_error "MCP is enabled but no client secret is available - omitting MCP keys from $CREDENTIALS_SECRET_NAME"
    fi
  fi
...
create_mcp_client() {
  local realm_name=$1
  local client_name="${MCP_CLIENT_ID:-mcp-client}"
  ...
  # Service account only - disable browser flows. create_client() sets neither
  # standardFlowEnabled nor implicitFlowEnabled, so a separate PUT is required.
  ...
  local sa_user_id=""
  if get_service_account_user_id "$realm_name" "$client_uuid"; then
    sa_user_id="$SA_USER_ID"
  elif [[ "$HTTP_CODE" != 404 ]]; then
    log_error "Service account lookup for '$client_name' failed (HTTP $HTTP_CODE) - leaving the existing client and its secret untouched"
    return 1
  else
    log_info "Client '$client_name' has no service account user - enabling serviceAccountsEnabled on an existing client does not create one retroactively"
    ...
    local delete_code
    delete_code=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$delete_url" \
      -H "Authorization: Bearer $ACCESS_TOKEN")
    if [[ ! "$delete_code" =~ ^2 ]]; then
      log_error "Failed to delete client '$client_name' for recreate (HTTP $delete_code)"
      return 1
    fi
    ...
    if [[ -z "$client_uuid" ]]; then
      log_error "Client '$client_name' not found after recreate"
      return 1
    fi

    flows_url="${KEYCLOAK_URL}/admin/realms/${realm_name}/clients/${client_uuid}"
    if curl_keycloak "$flows_url" "$flows_json" PUT; then
      log_success "Standard and implicit flows disabled for '$client_name'"
    else
      log_error "Failed to disable standard flow for '$client_name' (HTTP $HTTP_CODE)"
    ...
    sleep 2
    if ! get_service_account_user_id "$realm_name" "$client_uuid"; then
      log_error "Service account user '$sa_username' still not found after recreate (HTTP $HTTP_CODE)"
      return 1
    fi
    sa_user_id="$SA_USER_ID"
  fi
  ...
  # Project the MinIO client roles into the token so the EDP presigned-URL path authorizes.
  add_client_scope_mapper "$realm_name" "$client_name" "$client_name" "minio_roles" "minio_roles" "EnterpriseRAG-oidc-minio"

  # The MCP gateway is an OAuth resource server: it accepts only tokens carrying its own
  # audience, so callers cannot replay a token minted for some other service.
  add_client_audience_mapper "$realm_name" "$client_name" "$MCP_AUDIENCE"

  # Agents present this token on every tool call and refresh it themselves, so it can stay
  # short-lived without the realm-wide default (and every browser session) changing.
  set_client_access_token_lifespan "$realm_name" "$client_name" "$MCP_ACCESS_TOKEN_LIFESPAN"

  MCP_CLIENT_ID="$client_name"
  MCP_CLIENT_SECRET="$client_secret"
  export MCP_CLIENT_ID MCP_CLIENT_SECRET
  log_success "MCP client configured (client: $client_name)"
}
...
get_service_account_user_id() {
  local realm_name=$1
  local client_uuid=$2
  local url="${KEYCLOAK_URL}/admin/realms/${realm_name}/clients/${client_uuid}/service-account-user"

  SA_USER_ID=""

  local response http_code body
  for i in $(seq 1 $CURL_RETRY_LIMIT); do
    response=$(curl_get_with_status "$url")
    http_code=$(echo "$response" | tail -n1)
    body=$(echo "$response" | sed '$d')
    HTTP_CODE=$http_code

    if [[ "$http_code" =~ ^2 ]]; then
      SA_USER_ID=$(echo "$body" | jq -r 'if type == "object" then (.id // empty) else empty end')
      if [[ -n "$SA_USER_ID" ]]; then
        return 0
      fi
      log_error "Service account user lookup returned HTTP $http_code without an id"
      return 1
    fi
    ...
    log_info "Retry $i/$CURL_RETRY_LIMIT looking up service account user (HTTP $http_code)"
    sleep 2
  done
...
# Get user ID. The search endpoint is used rather than an unfiltered list because
# service-account users are omitted from the default /users listing.
get_user_id() {
  local realm_name=$1
  local username=$2

  local username_encoded
  username_encoded=$(jq -rn --arg u "$username" '@uri "\($u)"')
  local url="${KEYCLOAK_URL}/admin/realms/$realm_name/users?username=${username_encoded}&exact=true"

  local response
  response=$(curl_get "$url")

  if [[ "$(jq -r 'type' <<< "$response")" != "array" ]]; then
    log_error "User lookup for '$username' did not return a user list - Keycloak responded: ${response:0:200}"
    return 0
  fi

  jq -r '.[0].id // empty' <<< "$response"
}
...
# Add audience protocol mapper so access tokens issued for $client_name include
# $audience_client in the "aud" claim. Required for UMA permission requests
# (authz-keycloak plugin) where the resource server is a different client.
add_audience_mapper() {
  local realm_name=$1
  local client_name=$2
  local audience_client=$3
  local mapper_name="${4:-${audience_client}-audience}"
  local client_id
  client_id=$(get_client_id "$realm_name" "$client_name")
  local url="${KEYCLOAK_URL}/admin/realms/${realm_name}/clients/${client_id}/protocol-mappers/models"
  ...
  if curl_keycloak "$url" "$json"; then
    log_success "Audience mapper '$mapper_name' added to '$client_name' (aud=$audience_client)"
  elif [[ $HTTP_CODE == 409 ]]; then
    log_info "Audience mapper '$mapper_name' already exists on '$client_name'"
  else
    log_error "Failed to add audience mapper '$mapper_name' to '$client_name' (HTTP $HTTP_CODE)"
  fi
}
...
# Add an audience mapper so tokens issued to a client carry a given aud value. Needed for
# any resource server that rejects tokens not issued for it - a client_credentials token
# carries no usable aud of its own.
add_client_audience_mapper() {
  local realm_name=$1
  local client_name=$2
  local audience=$3
  ...
  if curl_keycloak "$url" "$json"; then
    log_success "Audience mapper '$audience' created for '$client_name'"
  elif [[ $HTTP_CODE == 409 ]]; then
    log_info "Audience mapper '$audience' already exists for '$client_name'"
  else
    log_error "Failed to create audience mapper '$audience' for '$client_name' (HTTP $HTTP_CODE)"
  fi
}

# Set the access token lifespan for a single client, leaving the realm default (and every
# browser session that follows it) untouched.
set_client_access_token_lifespan() {
  local realm_name=$1
  local client_name=$2
  local lifespan=$3

  local client_id=$(get_client_id "$realm_name" "$client_name")
  local url="${KEYCLOAK_URL}/admin/realms/${realm_name}/clients/${client_id}"
  local json='{"attributes": {"access.token.lifespan": "'$lifespan'"}}'

  if curl_keycloak "$url" "$json" PUT; then
    log_success "Access token lifespan for '$client_name' set to ${lifespan}s"
  else
    log_error "Failed to set access token lifespan for '$client_name' (HTTP $HTTP_CODE)"
  fi
}
...
# The frontend SPA client (EnterpriseRAG-oidc) issues user access tokens that
# APISIX exchanges for an UMA permission token against the resource-server
# client (EnterpriseRAG-oidc-backend). Keycloak rejects UMA permission requests
# with "invalid_token" / token endpoint 400 unless the access token's "aud"
# claim contains the resource server. Add an audience mapper to inject it.
log_info "Adding backend audience mapper to frontend client..."
add_audience_mapper "$KEYCLOAK_REALM" "EnterpriseRAG-oidc" "EnterpriseRAG-oidc-backend" "erag-backend-audience"
...
# Grafana OAuth client (SSO — same Keycloak session used by the ERAG UI)
if [ -n "${GRAFANA_DOMAIN:-}" ]; then
  log_info "Creating Grafana OAuth client..."
  if [ -n "${GRAFANA_PATH_PREFIX:-}" ]; then
    grafana_base_url="https://${GRAFANA_DOMAIN}${GRAFANA_PATH_PREFIX}"
    grafana_redirect_uri="https://${GRAFANA_DOMAIN}${GRAFANA_PATH_PREFIX}/login/generic_oauth"
  else
    grafana_base_url="https://$GRAFANA_DOMAIN"
    grafana_redirect_uri="https://$GRAFANA_DOMAIN/login/generic_oauth"
  fi

  # Confidential client with authorization code flow (standardFlow via create_client)
  create_client "$KEYCLOAK_REALM" "grafana-oauth" "false" "false" "false" "$grafana_base_url" "$grafana_redirect_uri" "true"

  # Register Grafana's post-logout redirect URIs
  grafana_client_uuid=$(get_client_id "$KEYCLOAK_REALM" "grafana-oauth")
  get_access_token
  HTTP_CODE=$(curl -sk -o /dev/null -w "%{http_code}" \
    -X PUT \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    -H "Content-Type: application/json" \
    -d '{"attributes": {"post.logout.redirect.uris": "'"${grafana_base_url}/login##${grafana_base_url}/"'"}}' \
    "${KEYCLOAK_URL}/admin/realms/${KEYCLOAK_REALM}/clients/${grafana_client_uuid}")
  if [[ "$HTTP_CODE" == "204" || "$HTTP_CODE" == "200" ]]; then
    log_success "Grafana OAuth post-logout redirect URIs set"
  else
    log_error "Failed to set Grafana post-logout redirect URIs (HTTP $HTTP_CODE)"
  fi
  ...
  if [ -n "${GRAFANA_OAUTH_CLIENT_SECRET:-}" ]; then
    ...
    HTTP_CODE=$(curl -sk -o /dev/null -w "%{http_code}" \
      ...
      "${KEYCLOAK_URL}/admin/realms/${KEYCLOAK_REALM}/clients/${local_client_uuid}")
    if [[ "$HTTP_CODE" == "204" || "$HTTP_CODE" == "200" ]]; then
      log_success "Grafana OAuth client secret set"
    else
      log_error "Failed to set Grafana OAuth client secret (HTTP $HTTP_CODE)"
    fi
  fi

  # Grafana role mapping: only admin/editor. Regular ERAG users are intentionally
  # NOT granted any Grafana role so that, combined with Grafana's
  # role_attribute_strict, only erag-admin and erag-maintainer can log in.
  create_client_role "$KEYCLOAK_REALM" "grafana-oauth" "admin"
  create_client_role "$KEYCLOAK_REALM" "grafana-oauth" "editor"

  # Map ERAG admin -> Grafana admin, maintainer -> editor (regular users: no access)
  assign_user_client_role "$KEYCLOAK_REALM" "erag-admin" "admin" "grafana-oauth"
  assign_user_client_role "$KEYCLOAK_REALM" "erag-maintainer" "editor" "grafana-oauth"

  # Add roles claim mapper so Grafana receives client roles in the token
  add_client_scope_mapper "$KEYCLOAK_REALM" "grafana-oauth-dedicated" "grafana-oauth" "roles" "grafana_roles" "grafana-oauth"

  # Map Grafana roles to groups (admin/maintainer only)
  map_client_role_to_group "$KEYCLOAK_REALM" "erag-admin-group" "grafana-oauth" "admin"
  map_client_role_to_group "$KEYCLOAK_REALM" "erag-maintainer-group" "grafana-oauth" "editor"
fi
...
if [[ "$OIDC_ENDPOINT" =~ ^https?:// ]]; then
  ...
  if [ -n "${GRAFANA_DOMAIN:-}" ]; then
    assign_role_realm_role "$KEYCLOAK_REALM" "ERAG-SSO-Admin" "grafana-oauth" "admin"
  fi
  ...
  if [ -n "${GRAFANA_DOMAIN:-}" ]; then
    assign_role_realm_role "$KEYCLOAK_REALM" "ERAG-SSO-Maintainer" "grafana-oauth" "editor"
  fi
