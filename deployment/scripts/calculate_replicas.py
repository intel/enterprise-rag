...
def verify_memory_requirements(current_vllm_size, reranking_size, embedding_size, node_memory_size, replicas, edp_enabled, telemetry_enabled, vector_databases_enabled, memory_overcommit_buffer_percent, vllm_memory_gib=42, embedding_memory_gib=4, reranking_memory_gib=4, num_routers=0, router_memory_gib=1):
    """
    ...
    Args:
        ...
        vllm_memory_gib: Memory per VLLM replica in GiB, derived from the model catalog
        embedding_memory_gib: Memory per embedding replica in GiB, derived from the model catalog
        reranking_memory_gib: Memory per reranking replica in GiB, derived from the model catalog
        num_routers: Number of KServe LLMInferenceService models being deployed
                     (one router-scheduler pod per model, independent of replica count).
                     Caller counts models with a router-producing runtime — typically
                     everything except OVMS. Default 0 = don't reserve for routers.
        router_memory_gib: Memory per router-scheduler pod in GiB. Two containers ×
                           500Mi requests by default = 1 GiB per pod.

    Returns:
        Tuple: (verified_replicas_count, memory_usage_percent)
    """

    # Per-role memory comes from the model catalog (models-rag.yaml `memory:`),
    # not hardcoded — matches the CPU sizing, which is already catalog-driven.
    VLLM_MEMORY      = vllm_memory_gib      if current_vllm_size > 0 else 0  # [GiB]
    EMBEDDING_MEMORY = embedding_memory_gib if embedding_size    > 0 else 0  # [GiB]
    RERANKING_MEMORY = reranking_memory_gib if reranking_size    > 0 else 0  # [GiB]
    ...
    # KServe router-scheduler pods: one per deployed LLMInferenceService model,
    # ~1 GiB each, NOT scaled by replicas. Caller (Ansible) passes the actual
    # model count from inference_models in config.erag.yaml.
    ROUTERS_MEMORY = num_routers * router_memory_gib

    # Set RAG core services memory request. Covers the platform + RAG-glue namespaces
    # that ship in every install (chatqna usvcs, istio, keycloak, envoy-gateway, kube-system,
    # auth-apisix, minio, postgresql, plus solutions-only additions kserve, lws-system,
    # cert-manager, metallb-system, envoy-ai-gateway-system, cnpg-system).
    CORE_SERVICES_MEMORY = 20  # [GiB]
    ...
    max_allocatable_memory = node_memory_size - CORE_SERVICES_MEMORY - OPTIONAL_SERVICES_MEMORY - ROUTERS_MEMORY - MEMORY_BUFFER - MEMORY_OVERCOMMIT_BUFFER
...
def calculate_replicas(nodes_dict, vllm_size, reranking_size, embedding_size, throughput_mode, edp_enabled, telemetry_enabled, vector_databases_enabled, memory_overcommit_buffer_percent, max_replicas_per_node, vllm_memory_gib=42, embedding_memory_gib=4, reranking_memory_gib=4, num_routers=0, router_memory_gib=1):
    """
    Calculate optimal replica distribution for VLLM, reranking, and embedding services.

    Args:
        nodes_dict: Dict with node names as keys and values containing: numa_nodes, cpus_per_numa_node, total_memory_GiB, amx_supported
    ...
    for node_name, node_data in nodes_dict.items():
        ...
        if node_data.get('amx_supported', False):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--embedding-memory-gib', type=int, default=4, help='Memory reserved per embedding replica in GiB, derived from model config (default: 4)')
    parser.add_argument('--reranking-memory-gib', type=int, default=4, help='Memory reserved per reranking replica in GiB, derived from model config (default: 4)')
    parser.add_argument('--num-routers', type=int, default=0, help='Number of KServe LLMInferenceService models being deployed. Each spawns one router-scheduler pod independent of replica count. Caller counts models with router-producing runtimes from inference_models in config.erag.yaml (typically all except OVMS). Default: 0 (no reservation).')
    parser.add_argument('--router-memory-gib', type=int, default=1, help='Memory per KServe router-scheduler pod in GiB. Default 1 = 2 containers × 500Mi requests.')
    ...
    results = calculate_replicas(
        ...
        args.embedding_memory_gib,
        args.reranking_memory_gib,
        args.num_routers,
        args.router_memory_gib
    )
