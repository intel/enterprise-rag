 ...
 * @returns {ServiceStatus} The determined status.
 */
export const determineServiceReadiness = (
  ...
  // The backend's status string is trusted to be one of ServiceStatus's
  // members at this API boundary; it isn't structurally guaranteed.
  return baseStatus as ServiceStatus;
};
...
export interface ParseServiceDetailsConfig<T extends string> {
  /**
   * Maps a service name to the graph node IDs it backs. A service backing more
   * than one instance of its kind lists a node ID per instance; a single node
   * ID is expressed as a one-element array.
   */
...
export const parseServiceDetails = <T extends string>(
  ...
  const statusEntries = Object.entries(annotations)
    ...
    .flatMap(([key, value]) => {
      ...
      const serviceNodeIds = serviceNameNodeIdMap[serviceName] ?? [];

      const status = determineServiceReadiness(value, key);

      return serviceNodeIds.map((serviceNodeId): [T, ServiceStatus] => [
        serviceNodeId,
        status,
      ]);
    });
  ...
  const metadataEntries = steps.flatMap(
    (step): [T, Record<string, string>][] => {
      ...
      return serviceNodeIds.map(
        (serviceNodeId): [T, Record<string, string>] => [
          serviceNodeId,
          metadata,
        ],
      );
