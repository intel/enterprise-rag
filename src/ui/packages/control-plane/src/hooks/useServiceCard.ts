...
export interface UseServiceCardOptions<T> {
  ...
  // Kind of the value model for a free-form instance key, forwarded on the
  // write so the backend can resolve it. Canonical services leave it unset.
  paramsKind?: string;
  filterFns?: {
...
export const useServiceCard = <T>(
  ...
  const { changeArguments, paramsKind, filterFns } = options;
    ...
    const changeArgumentsRequest = [
      {
        ...
        ...(paramsKind ? { params_kind: paramsKind } : {}),
      },
    ];
