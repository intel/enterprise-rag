 ...
 * @param {boolean} decode - Whether to percent-decode the value first (default: true).
 *   Pass false for values that must be preserved byte-for-byte, e.g. URLs, where decoding
 *   before storage/submission can corrupt the value (double-encoding, %2B vs literal +, etc.).
...
export const sanitizeString = (
  ...
  const valueToSanitize = decode ? tryDecode(value) : value;
  return DOMPurify.sanitize(valueToSanitize, {
    ALLOWED_TAGS: [],
    KEEP_CONTENT: true,
  });
};
