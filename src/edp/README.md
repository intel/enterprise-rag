...
Similar to other Enterprise RAG components, EDP has its own section under `env/<name>/config.erag.yaml`. All possible configuration options are listed in [deployment/components/edp/values.yaml](../../deployment/components/edp/values.yaml).

### Storage endpoints
Use the config.erag.yaml file to define the storage backend. Set the desired `storageType` under the edp section in your env config. Then, configure the appropriate sub-section based on the selected type.
...
Then deploy the application by following the instructions in [deployment/README.md](../../deployment/README.md). Deploy from the core installer root:
```bash
...
```bash
cd ../.. # src/ directory
docker build -f edp/Dockerfile -t erag/enhanced_dataprep .
```
...
docker run --rm -it --network edp-network --name edp-backend -p 5000:5000 -e DATABASE_HOST=edp-postgres -e DATABASE_USER=edp -e CELERY_BROKER_URL=redis://edp-redis:6379/0 -e CELERY_BACKEND_URL=redis://edp-redis:6379/0 erag/enhanced_dataprep
docker run -d --network edp-network --name edp-celery erag/enhanced_dataprep celery -A app.tasks.celery worker --loglevel=debug --concurrency=2
# (optional)
docker run -d --network edp-network --name edp-flower -p 5555:5555 erag/enhanced_dataprep celery -A tasks.celery flower
```
...
To enable system credentials fallback for presigned URLs generation, add the following to your `env/<name>/config.erag.yaml`:
...
Configure EDP for the S3 compatible storage by either exporting environment variables or setting values in your `env/<name>/config.erag.yaml`. If using an environment that requires proxy usage, ensure that you add `<your-IP-address>` to the `noProxy` variable in `env/<name>/global_config.yaml`.

Example `config.erag.yaml` snippet to configure S3 compatible storage:
