...
from app.utils import generate_presigned_url, gateway_unsafe_object_name_segments, get_local_minio_client, get_remote_minio_client, filtered_list_bucket, get_local_minio_client_using_token_credentials
from app.database import get_db
from app.models import FileResponse, FileStatus, LinkRequest, LinkResponse, LinkStatus, PresignedRequest, MinioEventData, S3EventData, PresignedResponse, SeaweedFSEventData, SharePointSiteItem, SharePointSitesResponse, SharePointSiteAddRequest, SharePointSiteRecord, RetrieveRequest
...
def presigned_url(input: PresignedRequest, request: Request) -> PresignedResponse:
    ...
    unsafe_segments = gateway_unsafe_object_name_segments(object_name)
    if unsafe_segments:
        logger.error(
            f"presigned_url: object name '{object_name}' has path segments the gateway "
            f"rewrites before the object store sees them: {unsafe_segments}"
        )
        raise HTTPException(
            status_code=400,
            detail="object_name must not contain empty, '.' or '..' path segments: the "
                   "gateway normalizes the path and the signature no longer matches"
        )
...
def api_file_text_extract(file_uuid: str, request: Request):
    ...
    with get_db() as db:
        ...
        try:
            ...
            # Files use multipart /upload endpoint; links use base JSON endpoint
            response = requests.post(
                TEXT_EXTRACTOR_ENDPOINT + "/upload",
...
async def api_retrieve(body: RetrieveRequest, request: Request):
    try:
        import requests
        ...
        embedding_request = { 'text': body.query }
        ...
        retriever_request['search_type'] = body.search_type
        retriever_request['k'] = body.k
        retriever_request['distance_threshold'] = body.distance_threshold
        retriever_request['fetch_k'] = body.fetch_k
        retriever_request['lambda_mult'] = body.lambda_mult
        retriever_request['score_threshold'] = body.score_threshold
        retriever_request['metadata_extraction_mode'] = body.metadata_extraction_mode
        retriever_request['search_by'] = body.search_by
        ...
        if body.reranker:
            ...
            reranker_request['top_n'] = body.top_n
            reranker_request['rerank_score_threshold'] = body.rerank_score_threshold

            logger.debug(f"Request to reranker: {reranker_request}")
            rerank_response = requests.post(RERANKER_ENDPOINT, json=reranker_request)
            if rerank_response.status_code != 200:
                return JSONResponse(status_code=500, content={'details': f"Reranker went wrong: {rerank_response.text}"})
            else:
                return JSONResponse(content={'docs': rerank_response.json()})
...
async def sharepoint_delete_file(request: Request):
    ...
    with get_db() as db:
        ...
        record = db.query(SharePointSiteRecord).filter(
            or_(
                SharePointSiteRecord.display_name == site_name,
                SharePointSiteRecord.name == site_name,
            )
        ).first()
        if not record:
            raise HTTPException(status_code=404, detail="SharePoint site not found for the given site_name.")
        site_name = site_display_name(record)

        exists = db.query(FileStatus).filter(
            FileStatus.site_name == site_name,
    ...
    if not exists:
        raise HTTPException(
            status_code=404,
            detail=f"File '{object_name}' not found on site '{site_name}', or it is already being deleted.",
        )
