...
class RetrieveRequest(BaseModel):
    ...
    search_type: Literal[
        "similarity",
        "similarity_search_with_siblings",
        "similarity_distance_threshold",
        "similarity_score_threshold",
        "mmr",
    ] = "similarity"
    k: int = Field(default=32, ge=1, le=1000)
    ...
    metadata_extraction_mode: str = Field(default="off", max_length=64)
    search_by: Dict[str, Any] = Field(default_factory=dict)
    reranker: bool = False
    top_n: int = Field(default=5, ge=1, le=100)
    rerank_score_threshold: float = Field(default=0.02, ge=0.0, le=1.0)
