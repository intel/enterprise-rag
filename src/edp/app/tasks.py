...
FINGERPRINT_ENDPOINT = os.environ.get('FINGERPRINT_ENDPOINT')  # per-key config read (GET /config)
FINGERPRINT_PIPELINE = os.getenv('FINGERPRINT_PIPELINE', 'dataprep')  # config scope the dataprep worker reads under
FINGERPRINT_TENANT = os.getenv('FINGERPRINT_TENANT', '_global')
FINGERPRINT_TIMEOUT_SECONDS = int(os.getenv('FINGERPRINT_TIMEOUT_SECONDS', '30'))
DPGUARD_PARAMS_KEY = 'dataprep_guard'  # params_key of the dataprep guardrail group
DPGUARD_WIRE_KEY = 'dataprep_guardrail_params'  # key the guardrail group nests under in the config value
...
def read_dpguard_params():
    """Read the dataprep guardrail parameters from the fingerprint service.

    Fetches the ``dataprep_guard`` group over the per-key config route, scoped
    to the dataprep pipeline, and returns the guardrail parameters the dpguard
    service expects. Returns ``None`` when the group has no stored row (the
    service answers 404), when the fingerprint service is unreachable, or when
    it returns a malformed body, so a transient config outage lets the guardrail
    fall back to its own defaults instead of failing the file. Programming
    errors are not swallowed — only request and response-decoding failures are.
    """
    try:
        response = requests.get(
            FINGERPRINT_ENDPOINT,
            params={
                'params_key': DPGUARD_PARAMS_KEY,
                'pipeline': FINGERPRINT_PIPELINE,
                'tenant': FINGERPRINT_TENANT,
            },
            timeout=FINGERPRINT_TIMEOUT_SECONDS,
        )
        if response.status_code == 404:
            logger.warning("No dataprep guard configuration found; using guardrail defaults.")
            return None
        if response.status_code != 200:
            logger.warning(f"Fingerprint returned {response.status_code} for dataprep guard config; using guardrail defaults. {response_err(response)}")
            return None
        return response.json()['values'][DPGUARD_WIRE_KEY]
    except requests.exceptions.RequestException as e:
        logger.error(f"Could not reach fingerprint service for dataprep guard config; using guardrail defaults. {e}")
        return None
    except (ValueError, KeyError) as e:
        logger.error(f"Malformed dataprep guard configuration from fingerprint; using guardrail defaults. {e}")
        return None
...
def _prepare_pdf_temp(file_data):
    ...
    try:
        ...
        os.close(tmp_fd)
        result = subprocess.run(  # nosec B603 # fixed argv, no shell; qpdf trusted local binary
...
def _parse_pdf_date(date_str):
    """Parse a PDF date (D:YYYYMMDDHHmmSS+HH'mm') into a UTC Unix timestamp."""
    if not date_str:
        return None
    text = date_str.strip().removeprefix("D:")
    digits = re.match(r"\d+", text)
    if not digits:
        return None
    stamp = digits.group(0)[:14].ljust(14, "0")
    try:
    ...
    if tz_match := re.search(r"([+-])(\d{2})'?(\d{2})?", text[len(digits.group(0)):]):
        sign = 1 if tz_match.group(1) == "+" else -1
        try:
            offset = datetime.timezone(sign * datetime.timedelta(
                hours=int(tz_match.group(2)), minutes=int(tz_match.group(3) or 0)))
        except (ValueError, OverflowError) as tz_err:
            logger.debug(f"Malformed timezone in PDF date '{date_str}', falling back to UTC: {tz_err}")
    return int(parsed.replace(tzinfo=offset).timestamp())


def _extract_pdf_metadata_qpdf(pdf_path):
    """Extract document-level PDF metadata (title, author, dates) using qpdf --json."""
    metadata = {}
    try:
        result = subprocess.run(  # nosec B603 # fixed argv, no shell; qpdf trusted local binary
            ['qpdf', '--json', pdf_path],
        ...
        # qpdf JSON v1: top-level "objects" dict, values are plain dicts
        # qpdf JSON v2: "qpdf" array, objects in qpdf[1], values wrapped in {"value": {...}}
        is_v2 = 'qpdf' in data and len(data.get('qpdf', [])) > 1
        if is_v2:
            objects = data['qpdf'][1]
        ...
        if is_v2:
            trailer = trailer.get('value', {})
        info_ref = trailer.get('/Info')
        if not info_ref or not isinstance(info_ref, str):
            return metadata

        # Resolve the info object (v1: key "N 0 R", v2: key "obj:N 0 R")
        info_obj = objects.get(info_ref) or objects.get(f"obj:{info_ref}") or {}
        if is_v2:
            info_obj = info_obj.get('value', {})

        if info_obj.get('/Title'):
            metadata['file_title'] = info_obj['/Title']
        if info_obj.get('/Author'):
            metadata['author'] = info_obj['/Author']
        elif info_obj.get('/Creator'):
            metadata['author'] = info_obj['/Creator']

        for pdf_key, out_key in [('/CreationDate', 'creation_date'), ('/ModDate', 'last_update_date')]:
            if (timestamp := _parse_pdf_date(info_obj.get(pdf_key))) is not None:
                metadata[out_key] = timestamp
    except Exception as e:
        logger.warning(f"PDF metadata extraction via qpdf failed for {pdf_path}: {e}")
        metadata = {}
...
def _split_pdf_part(src_path, file_name, file_id, part_index, max_pages, total_pages, total_parts):
    """Split one page range from a PDF using qpdf and return raw bytes."""
    ...
    try:
        try:
            subprocess.run(  # nosec B603 # fixed argv, no shell; qpdf trusted local binary
...
def _send_extraction_part(file_id, file_name, part_index, total_parts,
                          part_bytes, target_endpoint, timeout, response_key):
    """Send a single PDF part to an extractor pod via multipart/form-data and return (part_index, docs)."""
    ...
    resp = http.post(
        target_endpoint + "/upload", # multipart file endpoint; base path is for JSON link/text
        files={'files': (part_label, part_bytes)},
        timeout=timeout,
    )
...
def _split_and_extract_pipelined(file_id, file_name, file_data, max_pages,
    ...
    Args:
        ...
        prebuilt_parts: if set, skip splitting and use these raw-bytes parts directly
    """
    # ------------------------------------------------------------------
    # Phase 1: determine total parts and prepare temp file
    # ------------------------------------------------------------------
    original_pdf_metadata = {}
    if prebuilt_parts is not None:
        ...
        original_pdf_metadata = _extract_pdf_metadata_qpdf(_tmp_pdf_path)
        if original_pdf_metadata:
            logger.info(f"[{file_id}] Extracted original PDF metadata: {list(original_pdf_metadata.keys())}")
    ...
    for i in range(total_parts):
        for doc in results[i]:
            if isinstance(doc, dict) and isinstance(doc.get('metadata'), dict):
                ...
                # Inject original PDF metadata lost during splitting
                for key, value in original_pdf_metadata.items():
...
def process_file_task(self, file_id: Any, *args, **kwargs):
    ...
    if needs_split:
        ...
        try:
            with self.db_keepalive():
                upload_url = TEXT_EXTRACTOR_ENDPOINT + "/upload"
                response = http.post(upload_url, files={'files': (file_name, file_data)}, timeout=TEXT_EXTRACTOR_TIMEOUT_SECONDS)
        except requests.exceptions.ConnectionError as e:
            file_db.status = 'error'
            file_db.job_message = f"Connection error while calling text extractor service (service may have restarted). {e}"
            file_db.text_extractor_end = datetime.datetime.now()
    ...
    # Step 4.5 (Optional) - scan dataprepped documents with Dataprep Guardrail, after reading its configuration from fingerprint
    dpguard_msg = ""
    dpguard_status = ""
    try:
        ...
        if dpguard_enabled == "true":
            ...
            # Step 4.5.1 - read dataprep guard configuration from fingerprint
            dpguard_params = read_dpguard_params()
...
def process_link_task(self, link_id: Any, *args, **kwargs):
    ...
    # Step 4.5 (Optional) - scan dataprepped documents with Dataprep Guardrail
    dpguard_msg = ""
    dpguard_status = ""
    try:
        dpguard_enabled = os.getenv('DPGUARD_ENABLED', "false")
