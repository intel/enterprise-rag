...
var (
	...
	// Must replace every dot: controller-runtime derives the served path with
	// ReplaceAll, so a fixed replacement count breaks for any group whose dot
	// count differs and the webhook then 404s.
	validatingPath = fmt.Sprintf("/validate-%s-%s-%s", strings.ReplaceAll(apiGroup, ".", "-"), apiVersion, resource)
