...
from comps.guardrails.llm_guard_dataprep_guardrail.utils.llm_guard_dataprep_guardrail import EragLLMGuardDataprepGuardrail
...
def test_init(mock_dataprep_scanners_config, mock_usv_config):
    ...
    guardrail = EragLLMGuardDataprepGuardrail(mock_usv_config)
    mock_dataprep_scanners_config.assert_called_once_with(mock_usv_config)
    assert guardrail._scanners == ["BanSubstrings"]

@patch('comps.guardrails.llm_guard_dataprep_guardrail.utils.llm_guard_dataprep_guardrail.DataprepScannersConfig')
@patch('comps.guardrails.llm_guard_dataprep_guardrail.utils.llm_guard_dataprep_guardrail.EragLLMGuardDataprepGuardrail.scan_dataprep_docs')
def test_scan_llm_dataprep_valid(mock_scan_dataprep_docs, mock_dataprep_scanners_config, mock_dataprep_doc):
    ...
    guardrail = EragLLMGuardDataprepGuardrail({})
    doc = guardrail.scan_dataprep_docs(mock_dataprep_doc)

    assert doc == mock_dataprep_doc


@patch.object(EragLLMGuardDataprepGuardrail, '_scan_prompt')
@patch('comps.guardrails.llm_guard_dataprep_guardrail.utils.llm_guard_dataprep_guardrail.DataprepScannersConfig')
