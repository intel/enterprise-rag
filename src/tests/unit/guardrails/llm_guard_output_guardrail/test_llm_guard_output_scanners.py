...
from comps.guardrails.utils.llm_guard.input_scanners import BanSubstrings
from comps.guardrails.utils.llm_guard.output_scanners import JSON
...
def test_create_enabled_output_scanners_with_exception(mock_logger, output_scanners_config_instance):
    with patch('comps.guardrails.llm_guard_output_guardrail.utils.llm_guard_output_scanners.JSON', side_effect=Exception("Test Exception")):
        with pytest.raises(Exception) as e:
            ...
            assert "Some scanners failed to be created due to validation or unexpected errors. The details: {'json_scanner': 'An unexpected error occurred during creating output scanner json_scanner: Test Exception'}" in e.value.detail
