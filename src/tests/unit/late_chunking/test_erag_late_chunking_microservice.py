...
from comps.cores.proto.docarray import EmbedDocList, LateChunkingInput, TextDoc
...
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/late_chunking --cov-report=term --cov-report=html tests/unit/late_chunking/test_erag_late_chunking_microservice.py
...
@pytest.fixture
def mock_erag_late_chunking():
    """Mock the EragLateChunking class."""
    with patch('comps.late_chunking.utils.erag_late_chunking.EragLateChunking.__init__', autospec=True) as MockClass:
        MockClass.return_value = None
        yield MockClass


@pytest.fixture
def mock_erag_late_chunking_run():
    """Mock the EragLateChunking.run method."""
    with patch.object(EragLateChunking, 'run', return_value=EmbedDocList(docs=[])) as mock:
        yield mock
...
class TestMicroserviceDeclaration:
    ...
    def test_microservice_declaration_complies_with_guidelines(
        self, mock_load_dotenv, mock_erag_late_chunking, mock_cores_mega_microservice
    ):
        """Test that microservice follows ERAG guidelines."""
        try:
            import comps.late_chunking.erag_late_chunking_microservice as test_module
            importlib.reload(test_module)
        except Exception as e:
            pytest.fail(f"ERAG Late Chunking Microservice init raised {type(e).__name__} unexpectedly!")
...
class TestMicroserviceInitialization:
    ...
    def test_initialization_succeeds_with_defaults(
        self, mock_cores_mega_microservice, mock_erag_late_chunking_run
    ):
        ...
        # Assert that erag_late_chunking instance exists
        assert hasattr(test_module, 'erag_late_chunking'), "erag_late_chunking instance not found"
    
    def test_initialization_succeeds_with_env_vars_present(
        self, clean_env_vars, mock_erag_late_chunking_run
    ):
        ...
        with patch.dict("os.environ", {
            ...
            try:
                ...
                pytest.fail(f"ERAG Late Chunking Microservice init raised {type(e)} unexpectedly!")

            # Assertions to check the initialized values
            assert test_module.erag_late_chunking is not None, "erag_late_chunking instance not initialized"
    ...
    def test_initialization_with_custom_chunk_size(
        ...
        with patch.dict("os.environ", {
            ...
            assert test_module.erag_late_chunking is not None
...
class TestProcessFunction:
    ...
    @pytest.mark.asyncio
    async def test_process_succeeds(self, mock_cores_mega_microservice, mock_erag_late_chunking_run):
        ...
        mock_erag_late_chunking_run.return_value = mock_response
    ...
    async def test_process_handles_value_error(
        ...
        mock_erag_late_chunking_run.side_effect = ValueError("Test internal error")
    ...
    async def test_process_handles_general_exception(
        ...
        mock_erag_late_chunking_run.side_effect = Exception("Test unknown error")
