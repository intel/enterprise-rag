...
from comps.cores.proto.docarray import EmbedDocList, LateChunkingInput, TextDoc, TextDocList
from comps.late_chunking.utils.erag_late_chunking import EragLateChunking, _chunked_pooling


"""
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/late_chunking/utils/erag_late_chunking --cov-report=term --cov-report=html tests/unit/late_chunking/test_erag_late_chunking.py
...
def mock_chunker_class():
    ...
    with patch('comps.late_chunking.utils.erag_late_chunking.Chunker') as MockChunker:
        yield MockChunker


class TestEragLateChunkingInitialization:
    """Test EragLateChunking initialization."""
    ...
    def test_initialization_with_valid_parameters(self, mock_chunker_class, mock_requests_post):
        ...
        late_chunking = EragLateChunking(
...
class TestEragLateChunkingValidation:
