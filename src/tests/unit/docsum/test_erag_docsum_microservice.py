...
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/docsum --cov-report=term --cov-report=html tests/unit/docsum/test_erag_docsum_microservice.py
...
@pytest.fixture(autouse=True, scope="module")
def mock_cores_mega_statistics():
    with patch('comps.cores.mega.base_statistics.register_statistics') as mock_register, \
         patch('comps.cores.mega.base_statistics.statistics_dict', {}):
        mock_register.return_value = lambda func: func  # Return the function unchanged (decorator passthrough)
        yield mock_register
...
def test_microservice_declaration_complies_with_guidelines(mock_load_dotenv):
    """Test that the microservice declaration follows ERAG microservice conventions."""
    try:
        import comps.docsum.erag_docsum_microservice as test_module
        importlib.reload(test_module)
    except Exception as e:
        pytest.fail(f"ERAG Docsum Microservice init raised {type(e).__name__} unexpectedly!")
...
def test_initialization_succeeds_with_defaults():
    ...
    assert isinstance(test_module.erag_docsum.default_summary_type, str) and test_module.erag_docsum.default_summary_type, \
        "The default_summary_type is expected to be a non-empty string"

    # Assert that llm_usvc_endpoint is a non-empty string
    assert isinstance(test_module.erag_docsum.llm_usvc_endpoint, str) and test_module.erag_docsum.llm_usvc_endpoint, \
...
def test_initialization_succeeds_with_env_vars_present(clean_env_vars):
    ...
    with patch.dict("os.environ",
        ...
        try:
            ...
            pytest.fail(f"ERAG Docsum Microservice init raised {type(e)} unexpectedly!")

        # Assertions to check the initialized values
        assert test_module.erag_docsum.default_summary_type == "refine", "default_summary_type does not match"
        assert test_module.erag_docsum.llm_usvc_endpoint == "http://testhost:1234/v1", "llm_usvc_endpoint does not match"
...
def test_initialization_raises_exception_when_config_params_are_missing(clean_env_vars):
    ...
    with patch.dict("os.environ", {"DOCSUM_DEFAULT_SUMMARY_TYPE": ""}):
        with pytest.raises(Exception) as context:
            import comps.docsum.erag_docsum_microservice as test_module
            importlib.reload(test_module)
        # The EragDocsum class raises ValueError, but could be wrapped
...
@patch('comps.docsum.erag_docsum_microservice.EragDocsum.run')
