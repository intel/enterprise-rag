...
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/llms --cov-report=term --cov-report=html tests/unit/llms/test_erag_llm_microservice.py
...
def mock_validate_config():
   with patch('comps.llms.utils.erag_llm.EragLlm._validate_config', autospec=True) as MockClass:
      MockClass.return_value = MagicMock()
      yield MockClass
...
@pytest.fixture
def mock_EragLlm():
   with patch('comps.llms.utils.erag_llm.EragLlm.__init__', autospec=True) as MockClass:
...
def test_microservice_declaration_complies_with_guidelines(mock_load_dotenv, mock_get_connector, mock_validate_config, mock_cores_mega_microservice):
   try:
      import comps.llms.erag_llm_microservice as test_module
      importlib.reload(test_module)
   except Exception as e:
      pytest.fail(f"ERAG LLM Microservice init raised {type(e).__name__} unexpectedly!")
...
def test_initialization_succeeds_with_defaults(mock_get_connector, mock_cores_mega_microservice):
   ...
   assert isinstance(test_module.erag_llm._model_name, str) and test_module.erag_llm._model_name, "The model_name is expected to be a non-empty string"

   # Assert that _model_server is a non-empty string
   assert isinstance(test_module.erag_llm._model_server, str) and test_module.erag_llm._model_server, "The _model_server is expected to be a non-empty string"

   # Assert that _model_server_endpoint is a non-empty string
   assert isinstance(test_module.erag_llm._model_server_endpoint, str) and test_module.erag_llm._model_server_endpoint, "The _model_server_endpoint is expected to be a non-empty string"
...
def test_initialization_succeeds_with_env_vars_present(mock_get_connector, clean_env_vars):
   with patch.dict("os.environ",
      ...
      try:
         ...
         pytest.fail(f"ERAG LLM Microservice init raised {type(e)} unexpectedly!")

      # Assertions to check the initialized values
      assert test_module.erag_llm._model_name == "test_model_name", "Model name does not match"
      assert test_module.erag_llm._model_server == "test_model_server", "Model server does not match"
      assert test_module.erag_llm._model_server_endpoint == "http://testhost:1234", "Model server endpoint does not match"
      assert not test_module.erag_llm._disable_streaming, "Disable streaming flag should be unset"
...
@patch('comps.llms.erag_llm_microservice.EragLlm.run')
