...
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/language_detection --cov-report=term --cov-report=html tests/unit/language_detection/test_erag_language_detection_microservice.py
...
@pytest.fixture
def mock_EragLanguageDetector():
   with patch('comps.language_detection.utils.erag_language_detection.EragLanguageDetector.__init__', autospec=True) as MockClass:
      MockClass.return_value = None
      yield MockClass


@patch('dotenv.load_dotenv')
def test_microservice_declaration_complies_with_guidelines(mock_load_dotenv, mock_EragLanguageDetector, mock_cores_mega_microservice):
   try:
      import comps.language_detection.erag_language_detection_microservice as test_module
      importlib.reload(test_module)
   except Exception as e:
      pytest.fail(f"ERAG Language Detection Microservice init raised {type(e).__name__} unexpectedly!")
...
def test_initialization_succeeds_with_defaults(mock_cores_mega_microservice):
   ...
   assert not test_module.erag_language_detector._is_standalone, "This attribute should be False by default"
...
def test_initialization_succeeds_with_env_vars_present(mock_cores_mega_microservice, clean_env_vars):
   with patch.dict("os.environ",
      ...
      # Assert that _is_standalone is False
      assert test_module.erag_language_detector._is_standalone, "Attribute value does not match env variable"


@patch('comps.language_detection.utils.erag_language_detection.EragLanguageDetector.run')
