...
from comps.text_splitter.utils.erag_textsplitter import EragTextSplitter


@pytest.fixture
def reset_singleton():
    """Reset the singleton instance before each test."""
    EragTextSplitter._instance = None
    yield
...
def test_singleton_instance(reset_singleton):
    """Test that EragTextSplitter is a singleton."""
    splitter1 = EragTextSplitter(chunk_size=100, chunk_overlap=10, use_semantic_chunking=False)
    splitter2 = EragTextSplitter(chunk_size=200, chunk_overlap=20, use_semantic_chunking=True)
...
def test_empty_docs_raises_error(reset_singleton):
    ...
    splitter = EragTextSplitter(chunk_size=100, chunk_overlap=10, use_semantic_chunking=False)
...
@patch('comps.text_splitter.utils.erag_textsplitter.Splitter')
...
@patch('comps.text_splitter.utils.erag_textsplitter.SemanticSplitter')
def test_semantic_split_docs(MockSemanticSplitter, reset_singleton, mock_env_vars):
    ...
    splitter = EragTextSplitter(chunk_size=100, chunk_overlap=10, use_semantic_chunking=True)
...
def test_parameter_override(reset_singleton):
    ...
    with patch('comps.text_splitter.utils.erag_textsplitter.Splitter') as MockSplitter:
        mock_splitter_instance = MockSplitter.return_value
...
@patch('comps.text_splitter.utils.erag_textsplitter.sanitize_env')
def test_semantic_chunk_params_handling(mock_sanitize_env, reset_singleton):
    ...
    with patch('comps.text_splitter.utils.erag_textsplitter.SemanticSplitter') as MockSemanticSplitter:
    ...
    with patch('comps.text_splitter.utils.erag_textsplitter.SemanticSplitter') as MockSemanticSplitter:
        EragTextSplitter._instance = None  # Reset singleton
        mock_splitter = MockSemanticSplitter.return_value
        mock_splitter.split_text.return_value = ["chunk"]
...
@patch('comps.text_splitter.utils.erag_textsplitter.TableAwareSplitter')
