...
from comps.text_extractor.utils.erag_text_extractor import EragTextExtractor
from comps.cores.proto.docarray import TextDoc

@pytest.fixture
def reset_singleton():
    EragTextExtractor._instance = None
    yield
    EragTextExtractor._instance = None
...
def test_singleton_behavior(reset_singleton):
    instance1 = EragTextExtractor()
    instance2 = EragTextExtractor()
    assert instance1 is instance2

# Tests for load_data method

@patch('comps.text_extractor.utils.erag_text_extractor.logger')
def test_load_data_with_no_input(mock_logging, reset_singleton):
    ...
    text_extractor = EragTextExtractor()

    with pytest.raises(ValueError) as exc_info:
        text_extractor.load_data(files=[], link_list=[], texts=[])
        assert str(exc_info.value) == "No links and/or files passed for data preparation."

@patch.object(EragTextExtractor, '_load_files')
@patch.object(EragTextExtractor, '_load_links')
...
@patch('comps.text_extractor.utils.erag_text_extractor.FileParser')
@patch.object(EragTextExtractor, '_save_file_to_local_disk')
...
def test_load_files_cleanup(mock_remove, mock_exists, mock_save_file, mock_file_parser, reset_singleton, mock_uploadfile):
    ...
    mock_remove.assert_any_call("/tmp/test_document.txt")
...
@patch.object(EragTextExtractor, '_save_link_to_local_disk')
@patch.object(EragTextExtractor, '_load_text')
...
@patch.object(EragTextExtractor, '_save_link_to_local_disk')
@patch.object(EragTextExtractor, '_load_text')
@patch.object(EragTextExtractor, '_load_html_data')
