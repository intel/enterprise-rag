...
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/prompt_template --cov-report=term --cov-report=html tests/unit/prompt_template/test_erag_prompt_template_microservice.py
...
#    try:
#       import comps.prompt_template.erag_prompt_template_microservice as test_module
#       importlib.reload(test_module)
#    except Exception as e:
#       pytest.fail(f"ERAG Prompt Template Microservice init raised {type(e).__name__} unexpectedly!")
...
#    assert isinstance(test_module.erag_prompt_template.prompt_template, str) and test_module.erag_prompt_template.prompt_template, "The prompt_template is expected to be a non-empty string"