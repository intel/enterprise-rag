...
from comps.cores.proto.docarray import LLMPromptTemplate, PromptTemplateInput, TextDoc
...
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/prompt_template --cov-report=term --cov-report=html tests/unit/prompt_template/test_utils_erag_prompt_template.py
...
@pytest.fixture
def test_class():
    """Fixture to create EragPromptTemplate instance."""
    return EragPromptTemplate()
...
def test_erag_prompt_template_initialization_succeeds():
    # Assert that the instance is created successfully
    assert isinstance(EragPromptTemplate(), EragPromptTemplate), "Instance was not created successfully."


def test_erag_prompt_template_initialization_without_default_should_fail():
...
def test_erag_prompt_template_initializationwith_invalid_template_should_fail():
...
def test_erag_prompt_template_validate_suceeds(test_class, system_template, user_template, placeholders):
    try:
...
def test_erag_prompt_template_validate_raises_exception(test_class, system_template, user_template, placeholders, expected_message):
...
@pytest.mark.asyncio
async def test_erag_prompt_run_suceeds_with_defaults(test_class, mock_default_input_data, mock_default_response_data):
    try:
        result = await test_class.run(mock_default_input_data)
    except Exception as e:
        pytest.fail(f"ERAG Prompt Template Microservice init raised {type(e)} unexpectedly!")
...
async def test_erag_prompt_run_suceeds_with_chat_history(mock_get, mock_default_input_data_with_chat_history, mock_default_response_data_with_chat_history):
    ...
    test_class_with_history = EragPromptTemplate(chat_history_endpoint="http://test-endpoint")
...
@pytest.mark.asyncio
async def test_erag_prompt_run_suceeds_with_custom_prompt_template(test_class):
...
@pytest.mark.asyncio
async def test_erag_prompt_run_suceeds_with_dict_in_data(test_class):
    mock_input = PromptTemplateInput(
...
@pytest.mark.asyncio
async def test_erag_prompt_run_raises_exception_when_template_lacks_placeholders(test_class):
...
@pytest.mark.asyncio
async def test_erag_prompt_run_raises_exception_when_data_key_is_missing(test_class):
...
@pytest.mark.asyncio
async def test_erag_prompt_run_succeeds_with_empty_value_in_reranked_docs(test_class):
...
@pytest.mark.asyncio
async def test_erag_prompt_run_raises_exception_when_data_is_invalid(test_class):
    # step1
...
async def test_contextual_template_no_docs_with_history(mock_get):
    ...
    test_class = EragPromptTemplate(chat_history_endpoint="http://test-endpoint")
