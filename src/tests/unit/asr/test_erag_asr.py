...
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/asr --cov-report=term --cov-report=html tests/unit/asr/test_erag_asr.py
...
def mock_validate_model_server():
    ...
    with patch.object(EragAsr, '_validate_model_server', autospec=True) as mock_validate:
        yield mock_validate


def test_initialization_succeeds_with_valid_params(mock_validate_model_server):
    """Test that EragAsr initializes successfully with valid parameters."""
...
@patch('comps.asr.utils.erag_asr.requests.post')
...
async def test_run_succeeds_with_valid_json_response(mock_validate_model_server):
    ...
    with patch('comps.asr.utils.erag_asr.AsyncOpenAI') as mock_openai:
