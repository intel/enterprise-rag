...
To execute these tests with coverage report, navigate to the `src` folder and run the following command:
   pytest --disable-warnings --cov=comps/asr --cov-report=term --cov-report=html tests/unit/asr/test_erag_asr_microservice.py
...
def mock_validate_model_server():
    ...
    with patch('comps.asr.utils.erag_asr.EragAsr._validate_model_server', autospec=True) as mock_validate:
        yield mock_validate
...
def mock_validate_config():
    ...
    with patch('comps.asr.utils.erag_asr.EragAsr._validate_config', autospec=True) as mock_validate:
        yield mock_validate
...
def test_microservice_declaration_complies_with_guidelines(
    ...
    try:
        import comps.asr.erag_asr_microservice as test_module
        importlib.reload(test_module)
    except Exception as e:
        pytest.fail(f"ERAG ASR Microservice init raised {type(e).__name__} unexpectedly!")
    ...
    assert hasattr(test_module, 'erag_asr'), "erag_asr is not declared"
...
def test_initialization_succeeds_with_defaults(mock_validate_model_server, mock_cores_mega_microservice, set_valid_env_vars):
    ...
    assert isinstance(test_module.erag_asr._model_name, str) and test_module.erag_asr._model_name, \
        "The model_name is expected to be a non-empty string"

    # Assert that model_server is a non-empty string
    assert isinstance(test_module.erag_asr._model_server, str) and test_module.erag_asr._model_server, \
        "The model_server is expected to be a non-empty string"

    # Assert that model_server_endpoint is a non-empty string
    assert isinstance(test_module.erag_asr._model_server_endpoint, str) and test_module.erag_asr._model_server_endpoint, \
...
def test_initialization_succeeds_with_env_vars_present(mock_validate_model_server, clean_env_vars):
    ...
    with patch.dict("os.environ", {
        ...
        try:
            ...
            pytest.fail(f"ERAG ASR Microservice init raised {type(e)} unexpectedly!")

        # Assertions to check the initialized values
        assert test_module.erag_asr._model_name == "test_asr_model", "Model name does not match"
        assert test_module.erag_asr._model_server == "vllm", "Model server does not match"
        assert "http://testhost:8000" in test_module.erag_asr._model_server_endpoint, \
