...
@pytest.mark.smoke
@allure.testcase("IEASG-T715")
def test_edp_retrieve(edp_helper):
    """Ingest a file and retrieve it through /api/retrieve. Expect status code 200 and documents returned"""
    file_path = os.path.join(DATAPREP_UPLOAD_DIR, "file_in_public_bucket.txt")
    edp_helper.upload_file_and_wait_for_ingestion(file_path)

    response = edp_helper.retrieve({"query": "How many Hot Wheels cars does Lucianoooo have?", "k": 5})
    assert response.status_code == 200, f"Failed to retrieve documents. Response: {response.text}"
    docs = response.json().get("docs")
    assert docs, f"No documents returned. Response: {response.text}"
    logger.info(f"Retrieved documents: {docs}")


@allure.testcase("IEASG-T716")
def test_edp_retrieve_invalid_parameters(edp_helper):
    """Send /api/retrieve requests with invalid parameters. Expect status code 422 and no internal details leaked"""
    invalid_payloads = [
        {},                                                            # query is required
        {"query": ""},                                                 # query must not be empty
        {"query": "what is erag?", "k": 0},                            # k below minimum
        {"query": "what is erag?", "k": 1000000000},                   # k above maximum
        {"query": "what is erag?", "top_n": -1},                       # top_n below minimum
        {"query": "what is erag?", "lambda_mult": 5},                  # lambda_mult out of range
        {"query": "what is erag?", "search_type": "unsupported_type"},  # not an allowed search type
    ]
    for payload in invalid_payloads:
        response = edp_helper.retrieve(payload)
        assert response.status_code == 422, \
            f"Unexpected status code for payload {payload}. Response: {response.text}"
        assert "Traceback" not in response.text, \
            f"Internal details leaked for payload {payload}. Response: {response.text}"
...
def test_edp_regular_user_has_no_access_to_api(edp_helper, temporarily_remove_regular_user_required_actions):
    ...
    rbac_enabled = cfg.get("edp_rbac_enabled")
    if rbac_enabled:
...
def test_edp_upload_many_links_parallel_requests(edp_helper, temporarily_remove_brute_force_detection):
    ...
    shared_headers = edp_helper.get_headers().copy()

    def upload_single_link(link):
        return edp_helper.upload_links({"links": [link]}, headers=shared_headers)
