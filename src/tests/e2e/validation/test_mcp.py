...
def test_mcp_auth_requires_token(mcp_helper):
    """Verify that connecting without a bearer token is rejected.

    Opens a separate SSE connection (not the persistent test session) with no
    Authorization header. The MCP data path carries no anonymous access.
    """
    status = mcp_helper.connect_status(token=None)
    assert status == 401, f"Expected 401 without a token, got {status}"


@allure.testcase("IEASG-T708")
def test_mcp_auth_invalid_token(mcp_helper):
    """Verify that a token the gateway cannot validate is rejected."""
    status = mcp_helper.connect_status(token="not-a-real-token")
    assert status == 401, f"Expected 401 for an unverifiable token, got {status}"


@allure.testcase("IEASG-T709")
def test_mcp_session_is_bound_to_its_caller(mcp_helper, keycloak_helper):
    """Verify that knowing a session_id does not grant the ability to call tools.

    The session UUID travels in the query string and therefore reaches access logs, so
    the gateway must additionally require a token belonging to the identity that opened
    the session. A user token stands in for a different identity: it is not issued for
    the MCP audience, so it must be refused as well.
    """
    results = mcp_helper.probe_session_binding(foreign_token=keycloak_helper.access_token)
    logger.info(f"Session binding probe: {results}")
    assert results["endpoint"] == "/api/v1/mcp/messages/", \
        f"Gateway advertises an unexpected message endpoint: {results['endpoint']}"
    assert results["no_token"] == 401, \
        f"Tool call on a known session_id without a token returned {results['no_token']}"
    assert results["invalid_token"] == 401, \
        f"Tool call with an unverifiable token returned {results['invalid_token']}"
    assert results["foreign_token"] in (401, 403), \
        f"Tool call with another identity's token returned {results['foreign_token']}"
    assert results["owner"] == 202, \
        f"The session owner should still be accepted, got {results['owner']}"


@allure.testcase("IEASG-T710")
def test_mcp_protected_resource_metadata(mcp_helper):
    """Verify the OAuth protected-resource document agents use to find the token issuer.

    Reachable without a token by design - a client that has just been refused needs it to
    learn where to get one.
    """
    status, body = mcp_helper.get_protected_resource_metadata()
    assert status == 200, f"Metadata document returned {status}"
    logger.info(f"Protected resource metadata: {body}")
    assert body.get("resource", "").endswith("/api/v1/mcp"), \
        f"Unexpected resource identifier: {body.get('resource')}"
    assert body.get("authorization_servers"), "No authorization server advertised"
    assert "/realms/" in body["authorization_servers"][0], \
        f"Authorization server is not a Keycloak realm URL: {body['authorization_servers'][0]}"
