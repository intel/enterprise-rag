...
def _has_pipeline(pipeline_type: str) -> bool:
    ...
    return cfg.get("pipeline_type") == pipeline_type


@pytest.hookimpl
def pytest_markeval_namespace():
    """Make `cfg` resolvable inside the string `skipif` condition of requires_sso.

    pytest evaluates a string condition against the *test module's* globals plus
    whatever this hook contributes, so an importing module only needs the marker
    itself, not `cfg`.
    """
    return {"cfg": cfg}
...
requires_chatqna = pytest.mark.skipif(
    not _has_pipeline("chatqna"),
...
_KC_SSO_DEFAULT_ALIAS = "enterprise-sso"


def _kc_sso_alias() -> str:
    return cfg.get("erag_keycloak_oidc_alias") or _KC_SSO_DEFAULT_ALIAS
...
async def _perform_sso_login(
    ...
    sso_alias = _kc_sso_alias()
    sso_link = page.locator(f"#social-{sso_alias}")
    try:
        ...
        sso_link = page.locator(
            f'a:has-text("Enterprise SSO"), a:has-text("{sso_alias}")'
        )
...
async def sso_admin_helper(page, sso_admin_credentials):
    ...
    """
    base_url = cfg.get("base_domain_name")
...
requires_sso = pytest.mark.skipif(
    "not cfg.get('erag_keycloak_oidc_endpoint')",
    reason="SSO is not configured: erag_keycloak_oidc_endpoint is not set in "
           "env/<name>/config.rag.yaml",
)
...
async def chat_ui_helper(page, admin_credentials):
    ...
    chat_ui_helper = ChatUIHelper(page, base_url=cfg.get("base_domain_name"))
...
async def chat_ui_helper_maintainer(page, maintainer_credentials):
    ...
    helper = ChatUIHelper(page, base_url=cfg.get("base_domain_name"))
    await helper.login(
...
async def docsum_ui_helper(page, admin_credentials):
    ...
    docsum_helper = DocSumUIHelper(page, base_url=cfg.get("base_domain_name"))
...
async def audio_chat_ui_helper(page, admin_credentials):
    ...
    helper = AudioChatUIHelper(
        page, base_url=cfg.get("base_domain_name"), credentials=admin_credentials
    )
