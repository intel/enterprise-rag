...
# Skip all tests if audio is not enabled or chatqna is not deployed
_skip_marks = []
...
if cfg.get("pipeline_type") != "chatqna":
    _skip_marks.append(pytest.mark.skip(reason="ChatQnA pipeline is not deployed"))
if _skip_marks:
...
async def audio_chat_ui_helper(audio_page, admin_credentials):
    ...
    helper = AudioChatUIHelper(audio_page, base_url=cfg.get('base_domain_name'), credentials=admin_credentials)
