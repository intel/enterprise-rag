...
pytestmark = requires_chatqna

# The UI changes these settings on the chatqna pipeline, so the API cleanup must
# restore them in the same scope the chatqna router reads (chatqna._global).
PIPELINE = "chatqna"
TENANT = "_global"
    ...
    try:
        ...
        try:
            fingerprint_api_helper.set_component_parameters(
                "llm", max_new_tokens=1024, pipeline=PIPELINE, tenant=TENANT
            )
...
async def test_toggle_streaming_via_ui(chat_ui_helper, fingerprint_api_helper):
    ...
    try:
        ...
        # Reset streaming to the pipeline default (enabled) via API
        try:
            fingerprint_api_helper.set_component_parameters("llm", stream=True, pipeline=PIPELINE, tenant=TENANT)
            logger.info("Cleanup: stream reset to True via API")
        except Exception as e:
            logger.warning(f"Cleanup failed: {e}")
