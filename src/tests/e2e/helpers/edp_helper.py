...
class EdpHelper(ApiRequestHelper):
    ...
    def __init__(self, keycloak_helper, bucket_name=None):
        ...
        self.edp_api_path = f"https://{cfg.get('base_domain_name')}/api/v1/edp"
        if bucket_name:
    ...
    def upload_links(self, payload, as_user=False, headers=None):
        ...
        get_headers(). Parallel callers pass a single set so concurrent workers share one token.
        """
        logger.info(f"Attempting to upload links using the following payload: {payload}")
        response = requests.post(
            url=f"{self.edp_api_path}/links",
            headers=headers if headers is not None else self.get_headers(as_user),
            json=payload,
            verify=False
        )
        return response
    ...
    def retrieve(self, payload, as_user=False):
        """Make post call to /api/retrieve endpoint with the given payload"""
        logger.info(f"Attempting to retrieve documents using the following payload: {payload}")
        response = requests.post(
    ...
    def _open_and_send_file(self, file_path, presigned_url, as_user=False, auth_headers=None):
        logger.debug(f"Attempting to upload file {file_path} using presigned URL")
        try:
            headers = {}
            if (cfg.get("edp_storage_type", "seaweedfs") == "seaweedfs"
                    and cfg.get("edp_rbac_enabled", False)):
                source_headers = auth_headers if auth_headers is not None else self.get_headers(as_user)
                if "authorization" in source_headers:
                    headers["authorization"] = source_headers["authorization"]
    ...
    def upload_files_in_parallel(self, files_dir, file_names):
        ...
        # Presigned-URL uploads only carry a Keycloak token in the seaweedfs+RBAC mode.
        # When they do, fetch it once and share it so the workers do not each log in.
        needs_auth = (cfg.get("edp_storage_type", "seaweedfs") == "seaweedfs"
                      and cfg.get("edp_rbac_enabled", False))
        shared_headers = self.get_headers().copy() if needs_auth else None
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(file_names)) as executor:
            futures = [executor.submit(self._open_and_send_file, file_info['path'], file_info['presigned_url'], auth_headers=shared_headers) for file_info in files_info]
