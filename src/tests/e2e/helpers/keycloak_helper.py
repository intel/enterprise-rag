...
_DEFAULT_LOG_DIR = os.path.join(_CURRENT_DIR, "../../../../../../env/local/logs/rag")
_LOG_DIR = os.environ.get("ERAG_LOG_DIR", _DEFAULT_LOG_DIR)
...
class KeycloakHelper:
    ...
    def __init__(self, credentials_file, k8s_helper):
        ...
        if routing_mode == 'path':
            ...
            self.erag_auth_domain = f"https://keycloak.{fqdn}"
        ...
        self._admin_pass = self.k8s_helper.retrieve_admin_password(secret_name="keycloak-admin-secret", namespace="keycloak")
    ...
    def _perform_sso_login(self, sso_username, sso_password):
        ...
        """
        tenant_id = cfg.get("erag_keycloak_oidc_tenant_id")
        idp_alias = cfg.get("erag_keycloak_oidc_alias") or "enterprise-sso"

        if not tenant_id:
            raise RuntimeError("OIDC configuration (erag_keycloak_oidc_tenant_id) is required for SSO authentication")

        redirect_uri = f"https://{cfg.get('base_domain_name')}"
        keycloak_auth_url = (
