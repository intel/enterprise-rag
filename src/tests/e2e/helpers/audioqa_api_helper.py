...
class AudioApiHelper(ApiRequestHelper):
    ...
    def __init__(self, keycloak_helper):
        ...
        self.asr_api_path = f"https://{cfg.get('base_domain_name')}/v1/audio/transcriptions"
        self.tts_api_path = f"https://{cfg.get('base_domain_name')}/v1/audio/speech"
        self.tts = SherpaTTS()
    ...
    def transcribe_audio(self, audio_data, language="en", response_format="json", model=None, as_user=False, headers=None):
        """Send AudioData container to ASR service for transcription.

        headers: pre-fetched auth headers to reuse across concurrent calls; when None, each
        call fetches its own via get_headers().
        """
        files = {
        ...
        headers = (headers if headers is not None else self.get_headers(as_user)).copy()
    ...
    def text_to_speech(self, text: str, as_user=False, headers=None):
        """
        ...
        Args:
            ...
            headers: pre-fetched auth headers to reuse across concurrent calls; when None,
                each call fetches its own via get_headers()
