...
from comps.cores.mega.logger import (
    change_erag_logger_level,
    get_erag_logger,
)
...
logger = get_erag_logger("RAG Evaluator")
...
class Evaluator:
    ...
    def fetch_system_args(self) -> None:
        try:
            system_args = {}
            for params_key in ("llm", "retriever", "reranker"):
                response = self.fingerprint_api_helper.read_config(params_key)
                if response.status_code != 200:
                    raise ConnectionError(
                        f"Failed to read '{params_key}' config from the fingerprint API, "
                        f"status={response.status_code}")
                system_args.update(response.json().get("values", {}))
    ...
    def _check_connection_to_chatqa(self) -> bool:
        ...
        """
        logger.info("Checking connection to ChatQnA service...")
        try:
