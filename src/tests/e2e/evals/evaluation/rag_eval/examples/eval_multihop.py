...
def build_embeddings_client(endpoint: str, model_name: str | None = None):
    ...
    if not model_name:
        raise ValueError(
            "OpenAI-compatible embedding endpoint requires --embedding_model_name "
            "(the model name served by vLLM)."
        )
...
def args_parser():
    ...
    parser.add_argument("--embedding_model_name", type=str, default="nomic-ai/nomic-embed-text-v1", help="Model name served by vLLM at the embeddings endpoint. Only used for RAGAS metrics")
