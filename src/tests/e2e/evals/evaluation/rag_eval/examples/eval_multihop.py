...
logger = get_erag_logger("RAG Evaluator Multihop")
...
def build_embeddings_client(endpoint, model_name):
    """Return an OpenAI-compatible embeddings client (e.g. vLLM served at '.../v1/embeddings').
    The client can automatically detect the served model by querying <base>/v1/models
    """
    from langchain_openai import OpenAIEmbeddings

    normalized = endpoint.rstrip("/")
    if normalized.endswith("/v1/embeddings"):
        base_url = normalized[: -len("/embeddings")]
    elif normalized.endswith("/v1"):
        base_url = normalized
    else:
        raise ValueError(
            f"Unsupported embedding endpoint '{endpoint}'. "
            "Only OpenAI-compatible endpoints ending with '/v1' or '/v1/embeddings' (e.g. vLLM) are supported."
        )

    if not model_name:
        try:
            response = requests.get(f"{base_url}/models", timeout=10, verify=False)
            ...
            raise RuntimeError(
                f"Could not auto-detect embedding model name from {base_url}/models: {e}. "
                "Pass --embedding_model_name explicitly."
            ) from e
        if not served:
            raise RuntimeError(
                f"{base_url}/models returned no models. Pass --embedding_model_name explicitly."
            )
        if len(served) > 1:
            logger.warning(
                f"{base_url}/models advertises multiple models {served}; "
                f"using the first one ('{served[0]}'). Pass --embedding_model_name to override."
            )
        logger.info(f"Auto-detected embedding model from {base_url}/models: '{served[0]}'")
        model_name = served[0]

    # api_key is required by the client but ignored by vLLM
    logger.info(f"Using OpenAI-compatible embeddings client: base_url={base_url}, model={model_name}")
    ...
    def get_ragas_metrics(self, all_queries, arguments):
        ...
        try:
            embeddings = build_embeddings_client(
                ...
                model_name=getattr(arguments, "embedding_model_name", None),
            )
...
# Config file names expected inside the build config directory (kept in sync with tests/e2e/conftest.py)
BUILD_CONFIG_FILES = ["global_config.yaml", "config.erag.yaml", "config.inference.yaml"]


""" Validate auth file and cluster configuration directory exist"""
def validate_config_files_exists(args):
    ...
    if args.cluster_config_file:
        ...
        if not config_dir.is_dir():
            logger.error(f"Cluster configuration directory does not exist: {args.cluster_config_file}")
            raise FileNotFoundError(f"Cluster configuration directory not found: {args.cluster_config_file}")


""" Load cluster configuration from env/local YAMLs into singleton dict (cfg) - cluster information (FQDN, pipeline, proxies, etc.)"""
def load_cluster_configuration_file(args):
    build_config_dir = args.cluster_config_file
    if not build_config_dir or not os.path.isdir(build_config_dir):
        logger.warning(f"Cluster configuration directory {build_config_dir} does not exist. Using default configuration.")
        return cfg

    for filename in BUILD_CONFIG_FILES:
        filepath = os.path.join(build_config_dir, filename)
        if os.path.exists(filepath):
            logger.debug(f"Loading build configuration from {filepath}")
            with open(filepath, "r") as f:
                content = yaml.safe_load(f)
            if content:
                cfg.update(content)
        else:
            logger.warning(f"Build configuration file {filepath} not found; skipping.")
    return cfg
...
def args_parser():
    ...
    parser.add_argument("--embedding_endpoint", type=str, default="http://localhost:8108/v1/embeddings", help="URL of the vLLM OpenAI-compatible embeddings endpoint (must end with '/v1' or '/v1/embeddings'). Only used for RAGAS metrics")
    parser.add_argument("--embedding_model_name", type=str, default=None, help="Model name served at the embeddings endpoint. If not specified, the model is automatically detected from <endpoint>/v1/models. Only used for RAGAS metrics")
    ...
    parser.add_argument("--auth_file", type=str, default=os.path.abspath(os.path.join(os.environ.get("ERAG_LOG_DIR", os.path.join(get_project_root(), "..", "..", "..", "env/local/logs/rag")), "default_credentials.txt")), help="Path to auth file")
    parser.add_argument("--cluster_config_file", type=str, default=os.path.abspath(os.path.join(get_project_root(), "..", "..", "..", "env/local")), help=f"Path to directory containing cluster configuration YAMLs ({', '.join(BUILD_CONFIG_FILES)})")
