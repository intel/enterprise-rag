...
NAMESPACES = ["auth", "auth-apisix", "chat-history", "chatqna", "docsum", "edp", "erag-gateway", "fingerprint",
...
_LOG_DIR = os.environ.get("ERAG_LOG_DIR", "../../../../env/local/logs/rag")
...
def pytest_configure(config):
    ...
    if not build_config_dir or not os.path.isdir(build_config_dir):
        pytest.fail("Build configuration directory not found. Provide --build-config-dir with path to env/local/ configs.")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        pipeline_tag = f"pipeline:{ cfg.get('pipeline_type', 'unknown') }"
        llm_model_tag = f"llm_model:{ cfg.get('inference_models', [{}])[0].get('name', 'unknown') }"
...
def pytest_runtest_makereport(item, call):
    ...
    or an unmet skipif condition) and track them for removal from allure results.

    xfail results are deliberately NOT tracked: pytest reports an expected
    failure as ``report.skipped`` too, but sets ``wasxfail`` on it. Those must
    stay in the Allure report (as skipped with an "XFAIL <reason>" detail) so
    xfailed defects remain visible instead of vanishing from the report.
    """
    outcome = yield
    report = outcome.get_result()
    if report.skipped and not hasattr(report, "wasxfail") and call.when in ("call", "setup"):
...
def bootstrap_sso_user(keycloak_helper, suppress_logging, temporarily_remove_user_required_actions):
    ...
    """
    _oidc_keys = ("erag_keycloak_oidc_endpoint", "erag_keycloak_oidc_alias", "erag_keycloak_oidc_client_id",
                  "erag_keycloak_oidc_tenant_id", "erag_keycloak_oidc_client_secret")
    _missing_oidc_keys = [k for k in _oidc_keys if not cfg.get(k)]
    if _missing_oidc_keys:
        logger.info(f"OIDC not configured ({', '.join(_missing_oidc_keys)} not set) - skipping SSO user bootstrap")
        yield
        return
...
def disable_guards_at_startup(guard_helper, suppress_logging, temporarily_remove_user_required_actions):
    ...
    """
    # Guard helper targets the chatqna pipeline scope; skip on other pipeline types
...
def edp_cleanup_after_test(edp_helper):
    ...
    edp_enabled = cfg.get("edp_enabled")
    if not edp_enabled:
        yield
        return
...
def test_language():
    ...
    model_names = " ".join(m.get("name", "") for m in inference_models)
    if "PLLuM" in model_names or "Bielik" in model_names:
        return "pl"
