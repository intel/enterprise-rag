...
End-to-end performance benchmark for the Intel® AI for Enterprise RAG Document Summarization pipeline. Measures latency, throughput, and scaling characteristics under continuous load.
...
- Deployed Intel AI for Enterprise RAG cluster with DocSum pipeline active
- Network access (HTTPS) to the RAG deployment FQDN and Keycloak (or custom `ERAG_DOMAIN_NAME`)
...
# 3. Generate bearer token (reuse chatqna script)
../chatqna/generate_uat_to_file.sh .bearer 1
...
The `-m` flag specifies the HuggingFace model name used for token counting. It must match the model deployed in the RAG cluster so that input/output token counts are accurate.

```bash
# Use the exact model deployed in your RAG cluster:
...
Models are configured at deployment time via the pipeline resource files. The benchmark assumes the RAG cluster is already deployed with the desired model -- to change the LLM model, redeploy it.
