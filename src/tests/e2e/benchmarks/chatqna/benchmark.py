...
def parse_args():
    ...
    parser.add_argument("-s", type=str, default="https://solutions.ai/api/v1/chatqna", help="Server Address format")
...
def worker(wid, ctx, server, pool, result_queue, delay, tokenizer, bearer_file, warmup_req):
    ...
    while not ctx.is_set():
        question = pool.get()
        res = call_chatqna(server, question, wid, tokenizer, bearer)
...
def call_chatqna(url, question, wid, tokenizer, bearer):
