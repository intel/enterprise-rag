...
# Ensure proxy bypass is configured for the eRAG domain
...
if [[ $proxy_ok -eq 0 ]]; then
    echo "[INFO] Proxy bypass for '${ERAG_DOMAIN}' not found in no_proxy/NO_PROXY. Adding it now."
    export no_proxy="${no_proxy:+$no_proxy,}${ERAG_DOMAIN},.${ERAG_DOMAIN}"
    export NO_PROXY="${NO_PROXY:+$NO_PROXY,}${ERAG_DOMAIN},.${ERAG_DOMAIN}"
fi
...
count_vectors()
{
	...
	process_duration=0
	local http_code
	http_code=$(curl -k -s -o /tmp/_edp_files_resp.json -w "%{http_code}" "${EDP_URL}/files" -H "Authorization: Bearer ${USER_ACCESS_TOKEN}")
	upload_stat=$(cat /tmp/_edp_files_resp.json)

	if [[ "$http_code" != "200" ]]; then
		echo "[WARN] EDP /files returned HTTP $http_code: $upload_stat"
		echo "[WARN] Refreshing token and retrying..."
		refresh_uat
		http_code=$(curl -k -s -o /tmp/_edp_files_resp.json -w "%{http_code}" "${EDP_URL}/files" -H "Authorization: Bearer ${USER_ACCESS_TOKEN}")
		upload_stat=$(cat /tmp/_edp_files_resp.json)
		if [[ "$http_code" != "200" ]]; then
			echo "[ERROR] EDP /files returned HTTP $http_code after token refresh: $upload_stat"
			return 1
		fi
	fi
	...
	local json_type
	json_type=$(echo "$upload_stat" | jq -r 'type' 2>/dev/null)
	if [[ "$json_type" != "array" ]]; then
		echo "[ERROR] EDP /files response is not a JSON array (got: $json_type). Response: $upload_stat"
		return 1
	fi
...
wait_for_ingest()
{
	...
	while true
	do
		if ! count_vectors; then
			echo "[WARN] count_vectors failed, retrying in 30 seconds..."
			sleep 30
			continue
		fi
...
fill_with_wikipedia()
{
	...
	for ((fid=0; fid<$file_list_cnt; fid+=2)); do
		wait_for_ingest
		if ! count_vectors; then
			echo "[ERROR] Unable to get vector count, aborting"
			rm -rf $TEMP_DIR
			return 1
		fi
...
export EDP_URL="https://${ERAG_DOMAIN_NAME:-solutions.ai}/api/v1/edp"
...
refresh_uat
if ! count_vectors; then
	echo "[ERROR] Unable to connect to EDP, aborting"
	exit 1
fi
if [ "$DB_VECTORS" -gt "$EXPECTED_VECTORS" ]; then
	echo "Number of expected vectors are met, exiting"
