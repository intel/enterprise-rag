...
```bash
python erag_llm_guard_input_guardrail_microservice.py
```
...
```sh
cd ../../.. # src/ directory
docker build -t erag/in-guard:latest -f comps/guardrails/llm_guard_input_guardrail/impl/microservice/Dockerfile .
```

#### Run the Docker container, for example
```sh
docker run -d -e BAN_SUBSTRINGS_ENABLED=true -p 8050:8050 erag/in-guard:latest
```
