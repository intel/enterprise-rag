...
from comps.guardrails.utils.llm_guard.input_scanners import (
    Anonymize,
    BanSubstrings,
    BanTopics,
    Code,
...
from comps.guardrails.utils.llm_guard.input_scanners.ban_topics import (
...
from comps.guardrails.utils.llm_guard.input_scanners.code import (
    DEFAULT_MODEL as CODE_DEFAULT_MODEL
)
from comps.guardrails.utils.llm_guard.input_scanners.prompt_injection import (
    V1_MODEL as PROMPTINJECTION_V1_MODEL,
    V2_MODEL as PROMPTINJECTION_V2_MODEL,
    V2_SMALL_MODEL as PROMPTINJECTION_V2_SMALL_MODEL,
)
from comps.guardrails.utils.llm_guard.input_scanners.toxicity import (
...
class InputScannersConfig(ScannersConfigBase):

    _SCANNER_KIND = "input"

    def __init__(self, config_dict):
        self._logger = logger
        self._input_scanners_config = self._build_scanners_config_from_env(ENABLED_SCANNERS, config_dict)
...
#### METHODS FOR VALIDATING CONFIGS
    ...
    def _get_anonymize_config_from_env(self, config_dict):
        ...
        return self._scanner_config_from_env("anonymize", config_dict)
    ...
    def _get_invisible_text_config_from_env(self, config_dict):
        ...
        return self._scanner_config_from_env("invisible_text", config_dict)
    ...
    def _get_prompt_injection_config_from_env(self, config_dict):
        ...
        return self._scanner_config_from_env("prompt_injection", config_dict)
    ...
    def _get_secrets_config_from_env(self, config_dict):
        ...
        return self._scanner_config_from_env("secrets", config_dict)
    ...
    def _get_token_limit_config_from_env(self, config_dict):
        ...
        return self._scanner_config_from_env("token_limit", config_dict)
...
#### METHODS FOR CREATING SCANNERS
    ...
    def _create_token_limit_scanner(self, scanner_config):
        ...
        encoding_name = scanner_config.get('encoding_name', None)
    ...
    def create_enabled_input_scanners(self):
        ...
        """
        return self._create_enabled_scanners(self._input_scanners_config, self._create_input_scanner)
    ...
    def changed(self, new_scanners_config):
        ...
        """
        return self._config_changed(self._input_scanners_config, new_scanners_config)
