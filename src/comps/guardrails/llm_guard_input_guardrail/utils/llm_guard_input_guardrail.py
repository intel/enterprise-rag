...
from comps.cores.proto.docarray import LLMParamsDoc, LLMPromptTemplate
...
class EragLLMGuardInputGuardrail:
    """
    EragLLMGuardInputGuardrail is responsible for scanning and sanitizing LLM input prompts
    ...
    Methods:
        __init__(usv_config: dict):
            Initializes the EragLLMGuardInputGuardrail with the provided configuration.
    ...
    def _analyze_scan_outputs(self, prompt, results_valid, results_score):
        ...
        scanners_with_redact = ["BanSubstrings", "Regex"]
    ...
    def scan_llm_input(self, input_doc: LLMParamsDoc) -> LLMParamsDoc:
        ...
        if self._scanners:
            ...
            if "### Question:" in user_prompt:
                ...
                sanitized_user_prompt, user_results_valid, user_results_score = self._scan_prompt(user_prompt_to_scan)
                sanitized_user_prompt = prefix + sanitized_user_prompt + suffix
            else:
                sanitized_user_prompt, user_results_valid, user_results_score = self._scan_prompt(user_prompt)
