...
class Code(Scanner):
    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            ValueError: If the languages are not a subset of SUPPORTED_LANGUAGES.
        """
        ...
        self._chunk_separator = re.compile(r'\n\s*\n')
    ...
    def scan(self, prompt: str, output: str = None) -> tuple[str, bool, float]:
        prompt = output if output is not None else prompt
        if prompt.strip() == "":
        ...
        if len(code_blocks) == 0:
            ...
            code_blocks = [prompt]
            # Also split on blank-line boundaries to improve detection of mixed-language inputs
            chunks = [c.strip() for c in self._chunk_separator.split(prompt) if c.strip()]
            if len(chunks) > 1:
