...
> [!Warning]
> Dataprep guardrail is not enabled in default Chatqna's pipeline. To enable it, change `edp.dpGuard.enabled` to `true` in `env/<name>/config.erag.yaml`.
...
```bash
python erag_llm_guard_dataprep_guardrail_microservice.py
```
...
```sh
cd ../../.. # src/ directory
docker build -t erag/dpguard:latest -f comps/guardrails/llm_guard_dataprep_guardrail/impl/microservice/Dockerfile .
```

#### Run the Docker container, for example
```sh
docker run -d -e BAN_SUBSTRINGS_ENABLED=true -p 8070:8070 erag/dpguard:latest
```
