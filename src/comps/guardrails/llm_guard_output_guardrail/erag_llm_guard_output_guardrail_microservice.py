...
from comps.guardrails.llm_guard_output_guardrail.utils.llm_guard_output_guardrail import (
    EragLLMGuardOutputGuardrail
)


USVC_NAME = "erag_service@llm_guard_output_scanner"
logger = get_erag_logger("llm_guard_output_scanner_microservice") # TODO: to be changed to after folder structure changes
...
output_guardrail = EragLLMGuardOutputGuardrail(usvc_config)
...
async def process(llm_output: Request) -> Response: # GeneratedDoc or StreamingResponse
    """
    Process the LLM output using the EragLLMGuardOutputGuardrail.
