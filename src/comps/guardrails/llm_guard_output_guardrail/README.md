...
> [!Warning]
> Output guardrail is not enabled in default Chatqna's pipeline. To run a pipeline with output guardrails, check out custom pipelines in `deployment/pipelines/chatqna/variants/`.
...
```bash
python erag_llm_guard_output_guardrail_microservice.py
```
...
```sh
cd ../../.. # src/ directory
docker build -t erag/out-guard:latest -f comps/guardrails/llm_guard_output_guardrail/impl/microservice/Dockerfile .
```

#### Run the Docker container, for example
```sh
docker run --rm -p 8060:8060 --name="llm-guard-output-microservice" erag/out-guard:latest
```
