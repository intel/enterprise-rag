...
from comps.guardrails.utils.llm_guard.input_scanners.ban_topics import (  #input, becasue the same scanner to input and output
...
from comps.guardrails.utils.llm_guard.input_scanners.code import DEFAULT_MODEL as CODE_DEFAULT_MODEL
from comps.guardrails.utils.llm_guard.input_scanners.toxicity import DEFAULT_MODEL as TOXICITY_DEFAULT_MODEL
from comps.guardrails.utils.llm_guard.output_scanners import (
    BanTopics,
    Bias,
    Deanonymize,
    FactualConsistency,
...
from comps.guardrails.utils.llm_guard.input_scanners import BanSubstrings, Code, Regex
from comps.guardrails.utils.llm_guard.output_scanners.bias import DEFAULT_MODEL as BIAS_DEFAULT_MODEL
from comps.guardrails.utils.llm_guard.output_scanners.malicious_urls import DEFAULT_MODEL as MALICIOUS_URLS_DEFAULT_MODEL
from comps.guardrails.utils.llm_guard.output_scanners.no_refusal import DEFAULT_MODEL as NO_REFUSAL_DEFAULT_MODEL
from comps.guardrails.utils.llm_guard.output_scanners.relevance import (
...
from comps.guardrails.utils.llm_guard.vault import Vault

from comps.cores.mega.logger import get_erag_logger
from comps.cores.utils.utils import sanitize_env
from comps.guardrails.utils.scanners_config import ScannersConfigBase
...
class OutputScannersConfig(ScannersConfigBase):

    _SCANNER_KIND = "output"

    def __init__(self, config_dict):
        self._logger = logger
        self._output_scanners_config = self._build_scanners_config_from_env(ENABLED_SCANNERS, config_dict)
        self.vault = None
...
#### METHODS FOR VALIDATING CONFIGS
    ...
    def _get_url_reachability_config_from_env(self, config_dict):
        ...
        return self._scanner_config_from_env("url_reachability", config_dict)
...
#### METHODS FOR CREATING SCANNERS
    ...
    def _create_factual_consistency_scanner(self, scanner_config):
        ...
        if model_name is not None:
            if model_name in enabled_models:
                logger.info(f"Using selected model for FactualConsistency scanner: {model_name}")
                factual_consistency_params['model'] = enabled_models[model_name]
            else:
                err_msg = f"Model name is not valid for FactualConsistency scanner. Please provide a valid model name. Provided model: {model_name}. Enabled models: {list(enabled_models.keys())}"
                logger.error(err_msg)
                raise ValueError(err_msg)
    ...
    def create_enabled_output_scanners(self):
        ...
        """
        # ignore_non_dict: the output config also carries the Anonymize vault, which is
        # not a scanner configuration dictionary.
        return self._create_enabled_scanners(
            self._output_scanners_config,
            self._create_output_scanner,
            ignore_non_dict=True,
            vault=self.vault
        )
    ...
    def changed(self, new_scanners_config):
        ...
        """
        return self._config_changed(self._output_scanners_config, new_scanners_config, ignore_non_dict=True)
