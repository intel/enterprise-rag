...
```bash
python erag_prompt_template_microservice.py
```
...
```bash
cd ../../
docker build -t erag/prompt_template:latest -f comps/prompt_template/impl/microservice/Dockerfile .
```
...
When no custom prompt template is provided, the [default template](utils/erag_prompt_template.py) is automatically applied. This ensures that the system uses a standard structure for generating response for chat Q&A.
