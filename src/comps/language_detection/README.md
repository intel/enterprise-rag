...
```bash
python erag_language_detection_microservice.py
```
...
```bash
cd ../../
docker build -t erag/language_detection:latest -f comps/language_detection/impl/microservice/Dockerfile .
```
...
docker run -d --name="language-detection-microservice" \
  --net=host \
  --ipc=host \
  erag/language_detection:latest
```
