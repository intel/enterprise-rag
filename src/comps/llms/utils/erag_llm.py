...
from comps.cores.proto.docarray import GeneratedDoc, LLMParamsDoc
...
class EragLlm:
    def __init__(self, model_name: str, model_server: str, model_server_endpoint: str, insecure_endpoint: bool = False, disable_streaming: Optional[bool] = False, llm_output_guard_exists: Optional[bool] = True, headers: Optional[Dict[str, str]] = None):
        """
        Initialize the EragLlm instance with the given parameters.
        ...
        logger.info(
            f"ERAG LLM Microservice is configured to send requests to service {self._model_server_endpoint}"
        )
