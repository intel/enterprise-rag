...
@register_microservice(
    ...
    validate_methods=[erag_llm._connector._validate]
)
...
async def process(input: LLMParamsDoc) -> Response:
    """
    Processes the given LLMParamsDoc input using the ERAG LLM microservice.

    Args:
        input (LLMParamsDoc): The input parameters for the LLM processing.

    Returns:
        Response: The response from the ERAG LLM microservice.
    """
    start = time.time()
    try:
        # Pass the input to the 'run' method of the microservice instance
        res = await erag_llm.run(input)
...
async def get_models() -> JSONResponse:
    ...
    """
    try:
        models = await erag_llm.get_models()
