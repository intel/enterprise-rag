...
```bash
python erag_text_compression_microservice.py
```
...
```bash
cd ../.. # src/ directory
docker build -t erag/text_compression:latest -f comps/text_compression/impl/microservice/Dockerfile .
```
...
```bash
docker run -d --name="text_compression" --env-file comps/text_compression/impl/microservice/.env -p 9397:9397 erag/text_compression:latest
```
