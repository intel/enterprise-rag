...
# ONNXRuntime 1.28 pins each intra-op thread to a host CPU by default; under the
# pod's cpuset those pins fail (EINVAL), flooding logs at startup and spawning ~one
# thread per host core (oversubscription). RapidOCR (imported by pymupdf4llm below)
# and docling create ORT sessions that ignore DOCLING_NUM_THREADS/OMP_NUM_THREADS,
# so force an explicit thread count on every session — which also makes ORT skip the
# affinity pass. Must run before the pymupdf4llm/docling imports that build sessions.
import onnxruntime as _ort

_ORT_INTRA_OP_THREADS = int(os.getenv("DOCLING_NUM_THREADS") or os.getenv("OMP_NUM_THREADS") or 1)
_ort_orig_session_init = _ort.InferenceSession.__init__

def _ort_capped_session_init(self, *args, **kwargs):
    if len(args) >= 2:
        sess_options = args[1]
        args = (args[0],) + args[2:]
    else:
    ...
    if sess_options.intra_op_num_threads <= 0:
        sess_options.intra_op_num_threads = _ORT_INTRA_OP_THREADS
    kwargs["sess_options"] = sess_options
    _ort_orig_session_init(self, *args, **kwargs)

_ort.InferenceSession.__init__ = _ort_capped_session_init
...
# namespace prefixes required by ElementTree to query XMP packet XML (different from qpdf's object-level access)
_XMP_NS = {
...
def _parse_pdf_date(date_str):
    ...
    try:
        ...
        return None
    # Honour trailing UTC offset; absent means treat as UTC.
    offset = timezone.utc
    if tz_match := re.search(r"([+-])(\d{2})'?(\d{2})?", text[len(digits.group(0)):]):
        sign = 1 if tz_match.group(1) == "+" else -1
        try:
            offset = timezone(sign * timedelta(hours=int(tz_match.group(2)), minutes=int(tz_match.group(3) or 0)))
        except (ValueError, OverflowError) as e:
            logger.warning(f"Invalid timezone offset in PDF date '{date_str}': {e}")
    return int(parsed.replace(tzinfo=offset).timestamp())


def _parse_iso_date(date_str):
    """Parse an ISO-8601 date (e.g. 2026-07-22T05:52:12Z) into a UTC Unix timestamp."""
    if not date_str:
        return None
    text = date_str.strip()
...
def _extract_xmp_metadata(doc):
    """Fallback: read title/author/dates from the document's XMP packet."""
    try:
        xml = doc.get_xml_metadata()
    except Exception as e:
        logger.warning(f"Failed to read XMP metadata packet: {e}")
        return {}
    if not xml or not xml.strip():
        return {}
    try:
        from xml.etree.ElementTree import fromstring  # nosec B405 # XMP from PyMuPDF, not raw user input
        root = fromstring(xml)  # nosec B314 # XMP from PyMuPDF, not raw user input
    except Exception as e:
        logger.warning(f"Failed to parse XMP metadata XML: {e}")
        return {}

    def _first_text(*paths):
        for path in paths:
            for el in root.iterfind(path, _XMP_NS):
                for li in el.iterfind(".//rdf:li", _XMP_NS):
    ...
    result = {}
    if title := _first_text(".//dc:title"):
        result["title"] = title
    if author := _first_text(".//dc:creator"):
        result["author"] = author
    elif tool := _first_text(".//xmp:CreatorTool"):
        result["author"] = tool
    if created := _first_text(".//xmp:CreateDate", ".//dc:date"):
        if ts := _parse_iso_date(created):
            result["creation_date"] = ts
    if modified := _first_text(".//xmp:ModifyDate", ".//xmp:MetadataDate"):
        if ts := _parse_iso_date(modified):
            result["last_update_date"] = ts
    return result
...
def _build_docling_converter():
    ...
    # Cap docling/ONNXRuntime thread pool. Without this, ORT queries the host's
    # hardware_concurrency() (bypasses the pod's cpuset) and spawns ~one
    # intra-op thread per host core — on high-core boxes that OOMs the pod
    # because each thread reserves its own ORT arena. Read the thread count
    # explicitly (DOCLING_NUM_THREADS, falling back to OMP_NUM_THREADS) so it is
    # independent of docling's pydantic-settings env parsing; cluster admins tune
    # it via configmap without a code change.
    num_threads = int(os.getenv("DOCLING_NUM_THREADS") or os.getenv("OMP_NUM_THREADS") or 4)
    pipeline_options.accelerator_options = AcceleratorOptions(device=AcceleratorDevice.CPU, num_threads=num_threads)
...
def _save_image(data, save_path="/tmp/erag_upload"):
...
class LoadPdf(AbstractLoader):
    ...
    def extract_text(self):
        ...
        erag_text_extractor_microservice.py bypasses LoadPdf entirely and calls
    ...
    def extract_metadata(self):
        ...
        Info dictionary first, then XMP packet for anything still missing.
        """
        ...
        try:
            ...
            try:
                ...
                xmp = _extract_xmp_metadata(doc)
            finally:
                doc.close()

            if title := (meta.get('title') or xmp.get('title')):
                metadata['file_title'] = title

            # /Author is the document author while /Creator is the authoring
            # application so it is only used when nothing better exists
            if author := (meta.get('author') or xmp.get('author') or meta.get('creator')):
                metadata['author'] = author

            for pdf_key, out_key in [('creationDate', 'creation_date'), ('modDate', 'last_update_date')]:
                if (timestamp := _parse_pdf_date(meta.get(pdf_key))) is None:
                    timestamp = xmp.get(out_key)
