...
| `ERAG_LOGGER_LEVEL`               | `INFO`                                             | Log level for the microservice. Supported: DEBUG, INFO, WARNING, ERROR       |
...
```bash
python erag_query_rewrite_microservice.py
```
...
```bash
cd ../../
docker build -t erag/query-rewrite:latest -f comps/query_rewrite/impl/microservice/Dockerfile .
```
