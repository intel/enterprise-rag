...
```bash
python erag_ingestion_microservice.py
```
...
```bash
cd ../.. # src/ directory
docker build -t erag/ingestion:latest -f comps/ingestion/impl/microservice/Dockerfile .
```
...
```bash
docker run -d --name="ingestion" --env-file comps/ingestion/impl/microservice/.env -p 6120:6120 erag/ingestion:latest
```
