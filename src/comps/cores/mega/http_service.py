...
from .logger import get_erag_logger


class BaseService:
    """Base service class that provides common interface."""

    def __init__(self, runtime_args: Optional[dict] = None):
        """Initialize BaseService with runtime arguments."""
        runtime_args = runtime_args or {}
        self.title = runtime_args.get("title", "GenAI Microservice")
        self.description = runtime_args.get("description", "Microservice Infrastructure")
        self.host_address = runtime_args.get("host", "0.0.0.0")
        self.primary_port = runtime_args.get("port", 8080)
        self.logger = get_erag_logger(self.title)
