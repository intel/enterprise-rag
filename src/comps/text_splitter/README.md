...
|----------------------------------|---------------------------|--------------------------------------------------------------------------------------------------|
| `ERAG_LOGGER_LEVEL`              | `INFO`                    | Microservice logging output level                                                                |
...
| `EMBEDDING_SERVICE_ENDPOINT`     | `http://embedding-svc.chatqna.svc:6000/v1/embeddings`  | Endpoint of the embedding microservice used for semantic chunking. For more details about the microservice, see [src/comps/embedding/README.md](../embeddings/README.md)                                  |
...
```bash
python erag_text_splitter_microservice.py
```
...
```bash
cd ../.. # src/ directory
docker build -t erag/text_splitter:latest -f comps/text_splitter/impl/microservice/Dockerfile .
```
...
```bash
docker run -d --name="text_splitter" --env-file comps/text_splitter/impl/microservice/.env -p 9399:9399 erag/text_splitter:latest
```
  ...
  "docs": [
    {
      ...
      "metadata": {
        ...
        "filename": "/tmp/erag_upload/test.txt",
