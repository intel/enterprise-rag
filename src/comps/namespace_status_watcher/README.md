...
| `ERAG_LOGGER_LEVEL`         | Logger level for the microservice                            | INFO                  |
...
```bash
python erag_namespace_status_watcher_microservice.py
```
...
```bash
cd ../../
docker build -t erag/namespace-status-watcher:latest -f comps/namespace_status_watcher/impl/microservice/Dockerfile .
```
...
docker run -d --name namespace-status-watcher-microservice \
  --net=host \
  --ipc=host \
  erag/namespace-status-watcher:latest
```
