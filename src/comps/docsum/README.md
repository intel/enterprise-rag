...
| `DOCSUM_MODEL_CONTEXT_LENGTH`   | Model context length in tokens, used to reject a `stuff` request that would not fit before summarization starts. Unset by default, in which case the value is read from the LLM's `/v1/models` (see the note below). |
| `ERAG_LOGGER_LEVEL`             | Log level for the microservice. Supported values: DEBUG, INFO, WARNING, ERROR. Default is INFO.                       |

> [!NOTE]
> `/v1/models` reports `max_model_len` only when the LLM microservice talks to the model
> server (for example vLLM) directly. When an AI gateway sits in between, it serves its
> own model list without that field, and the microservice logs
> `Model context length not available from /v1/models; token pre-validation will be
> skipped.` Set `DOCSUM_MODEL_CONTEXT_LENGTH` to the model's context length to keep the
> pre-validation. Without it a `stuff` request larger than the context window is
> rejected by the model server mid-summarization instead.
...
```bash
python erag_docsum_microservice.py
```
...
```bash
cd ../../
docker build -t erag/docsum:latest -f comps/docsum/impl/microservice/Dockerfile .
```
