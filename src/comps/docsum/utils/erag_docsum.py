...
from comps.cores.proto.docarray import GeneratedDoc, TextDocList
...
class EragDocsum:
    """ERAG Document Summarization Component."""
    def __init__(self, llm_usvc_endpoint: str, default_summary_type: str = "map_reduce", max_concurrency: int = 16,
        ...
        """
        Initialize the EragDocsum component.
        ...
        Args:
            ...
            max_concurrency (int): Maximum number of concurrent requests to the LLM service. Default is 16.
                                   ...
                                   in map_reduce mode.
            model_context_length (Optional[int]): The model's maximum context length in tokens. When not given,
                                   it is looked up in /v1/models, which only reports it when the LLM
                                   microservice talks to the model server directly - an AI gateway in
                                   between serves its own model list without that field.
        """
        ...
        if model_context_length is not None and model_context_length < 1:
            raise ValueError("model_context_length must be a positive number of tokens.")
        ...
        if model_context_length is not None:
            ...
            logger.info(f"Model context length taken from configuration: {model_context_length} tokens")
        else:
    ...
    def _validate(self) -> None:
        ...
        logger.info("EragDocsum configuration validated successfully.")
        ...
        try:
            ...
            if model_context_length is None:
                logger.info("Model context length not available from /v1/models; token pre-validation will be skipped. "
                            "Set DOCSUM_MODEL_CONTEXT_LENGTH to enable it.")
