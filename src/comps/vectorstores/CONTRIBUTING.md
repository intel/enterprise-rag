...
- Implement the required interface methods based on [VectorStoreConnector](utils/connectors/connector.py) abstract class
...
Add your connector to the connector mapping in the `utils/erag_vectorstore.py` file:
...
- implement the `vector_database` ansible task in `deployment/roles/app_vector_databases/`:
...
- implement proper ansible values.yaml override file in `deployment/roles/app_vector_databases/templates/values.yaml.j2` that generates a compatible `values.yaml` file via ansible 
- verify that the `vector_database` component is deployable by configuring and deploying the solution via `env/<name>/config.erag.yaml`
...
This native monitoring can complement the observability system (deployed by the Enterprise AI Solutions installer) to provide comprehensive observability of your vector database operations.
