...
# OAuth 2.0 resource-server settings. Agents authenticate with their own Keycloak token.
# The edge SecurityPolicy verifies it too; verifying here as well means a request that
# arrives by any other route is held to the same standard.
JWT_ISSUER = os.getenv("MCP_JWT_ISSUER", "")
JWT_AUDIENCE = os.getenv("MCP_JWT_AUDIENCE", "")
JWKS_URI = os.getenv("MCP_JWKS_URI", "")
JWT_ALGORITHMS = [alg.strip() for alg in os.getenv("MCP_JWT_ALGORITHMS", "RS256,RS384").split(",") if alg.strip()]
RESOURCE_URL = os.getenv("MCP_RESOURCE_URL", "")

HEALTH_PATH = "/api/v1/mcp/health"
METADATA_PATH = "/api/v1/mcp/.well-known/oauth-protected-resource"

_S3_SSL_CONTEXT = build_ssl_context(S3_TLS_VERIFY)

# The MCP SDK validates the Host header to prevent DNS rebinding attacks. Its matcher
# (mcp.server.transport_security) supports an exact match or a trailing ":*" port
# wildcard only - there is no "*.domain" glob - so a hostname reachable both with and
# without an explicit port needs two entries. Loopback is always allowed so that
# port-forward and local runs work; the deployment's own hostnames come from
# MCP_ALLOWED_HOSTS (comma-separated) and are appended.
...
logger.info(f"Host header allow-list: {ALLOWED_HOSTS}")
...
# message_path is the path *within* this app: the SSE transport advertises it to clients as
# scope["root_path"] + message_path, and the Mount below supplies "/api/v1/mcp" as the root
# path. Spelling the full prefix here would advertise /api/v1/mcp/api/v1/mcp/messages/.
mcp = FastMCP(
    "erag-mcp",
...
if EDP_ENDPOINT:
    ...
    async def ingest_file(
        ...
        try:
            ...
            # EDP answers /api/presignedUrl in one of two shapes. In advanced-IAM mode it
            # returns an unsigned direct URL and the storage backend validates the Bearer
            # token at request time, so the header has to travel with the upload. In the
            # MinIO branch the URL is SigV4-presigned and already carries its own
            # authorization - forwarding the token there would expose it to the object
            # store for no gain, so it is withheld.
            upload_headers = {"Content-Type": content_type}
            if "X-Amz-Signature" not in put_url:
...
async def protected_resource_metadata(_: StarletteRequest) -> JSONResponse:
    """OAuth 2.0 Protected Resource Metadata (RFC 9728).

    Advertised through the resource_metadata parameter of the WWW-Authenticate challenge
    on every 401, so an MCP client can discover which authorization server to get a token
    from and refresh on its own once the current one expires.
    """
    return JSONResponse({
        "resource": RESOURCE_URL,
        "authorization_servers": [JWT_ISSUER] if JWT_ISSUER else [],
...
app = BearerAuthMiddleware(
    _starlette_app,
    verifier=_verifier,
    resource_metadata_url=f"{RESOURCE_URL}/.well-known/oauth-protected-resource" if RESOURCE_URL else "",
    public_paths={HEALTH_PATH, METADATA_PATH},
...
if __name__ == "__main__":
    if not JWT_ISSUER or not JWT_AUDIENCE:
        raise RuntimeError(
            "MCP_JWT_ISSUER and MCP_JWT_AUDIENCE must be set - without them the gateway "
            "cannot tell which tokens were issued for it"
        )
    if not JWKS_URI:
        raise RuntimeError("MCP_JWKS_URI must be set - caller tokens cannot be verified without it")
